...
def authenticate_and_get_token():
    ...
    auth_data = {
        'email': 'sam.mueller@ssc-spc.gc.ca',
        'password': 'adminadmin'
    }
...
def api_cleanup(token):
    ...
    if response.status_code == 200:
        ...
        print(f"   Current state: {vector_db.get('total_collections', 0)} total collections")
    ...
    if response.status_code == 200:
        ...
        if 'cleanup_results' in result:
            ...
            print(f"   - Web search: {web_search.get('collections_cleaned', 0)} cleaned")
