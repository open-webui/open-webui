...
- 📱 **Pull-to-Refresh Support for iOS PWA**: Added pull-to-refresh functionality for iOS Progressive Web App users, allowing them to easily refresh the interface by pulling down on the navigation area. This resolves timeout issues that occurred when temporarily switching away from the app during long AI response generations, eliminating the need to close and relaunch the PWA.
