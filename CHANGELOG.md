...
## [Fork] Merged upstream v0.9.5 - 2026-05-11

Synchronized this fork with upstream Open WebUI v0.9.5 while preserving local customizations.

### Taken from upstream
- All v0.9.5 security fixes (SSRF protection, iframe CSP, mass-assignment hardening, ownership checks, public-sharing permission gates, image URL validation, model params stripping for read-only users)
- Channel streaming agent — channels now flow through the full chat completion pipeline (native/default tool calling, built-in tools, user tools, MCP, filters, RAG)
- AsyncSession migration across models, routers, and helpers
- New schema columns and tables (shared chat, pinned notes, automations, calendar) plus backing Alembic migrations
- New unstructured loaders (Excel, PowerPoint), structural detail-block grouping in markdown rendering, scheduler worker loop, OAuth token refresh as async, `GOOGLE_OAUTH_AUTHORIZE_PARAMS` config
- `ENV UV_LINK_MODE=copy` Dockerfile fix for QEMU arm64 cross-builds

### Kept local
- Pinecone namespace isolation, retry logic, JSONB-backed indexes, and PG17 `JSON_TABLE` search optimizations
- Gmail auto-sync (signup + periodic), Google Drive knowledge sync, OAuth scope additions, Gmail attachment processing, Gmail performance tuning knobs
- Spend-limit middleware and `UserUsages` recording
- Video/audio chapter generation, transcript segments with word-level timestamps, semantic chunking strategy
- Custom branding (`WEBUI_NAME` left as-is, custom favicons), ReportLab-based PDF export with custom fonts, Word document export
- Knowledge sources display on channel messages, RAG context injection in channels, knowledge tab in channel input menu
- Robust CSV loader, `unstructured[all-docs]` extra, `reportlab`, `rq`, retained extra Google API libs
- Alembic merge migration (`f7e8d9c0b1a2`) unifying upstream and local heads after renaming the colliding `e1f2a3b4c5d6_add_is_pinned_to_note.py` to `e2f3a4b5c6d7`
- Multi-head `alembic upgrade heads` and pre-upgrade overlapping-version cleanup logic in `config.py`
- Temp file cleanup scheduler (Render `/tmp` 2GB limit)
- GCS retry/timeout config and signed URLs for streaming
- "Improve Prompt" button + `/api/v1/tasks/prompt/improve` endpoint
- Mobile-friendly placeholder layout, artifact CSV/JSON detection
