...
- **DEFAULT_USER_CHAT_MODE Configuration**: Introduced the DEFAULT_USER_CHAT_MODE environment variable to control the default chat mode (temporary or normal) for users. This allows a more customizable user experience, enabling users to have chats default to a temporary mode or normal mode based on the environment variable.
