...
- 🎯 **Code block token comparison.** Chat message updates during streaming are now faster thanks to an O(1) fast-path check on token text fields before falling back to expensive JSON.stringify comparison. [#22101](https://github.com/open-webui/open-webui/pull/22101)
- 📊 **Status history comparison.** Chat message updates during streaming are now faster thanks to an O(1) array length check before expensive JSON.stringify comparison for status history. [#22103](https://github.com/open-webui/open-webui/pull/22103)
- 🛠️ **Tool result cloning.** Tool execution results are now cloned more efficiently using structuredClone() instead of JSON.parse(JSON.stringify(...)), improving streaming performance. [#22104](https://github.com/open-webui/open-webui/pull/22104)
- 💾 **Cold path cloning optimization.** Model selection, file preparation, and history saving are now faster thanks to structuredClone() replacing JSON.parse(JSON.stringify(...)) in 7 locations. [#22102](https://github.com/open-webui/open-webui/pull/22102)
...
- 🗄️ **Socket DB call optimization.** All synchronous database calls in the socket event emitter are now run in background threads via asyncio.to_thread(), preventing blocking of the event loop during streaming and improving performance for concurrent users. [#22087](https://github.com/open-webui/open-webui/pull/22087)
- 📝 **Status event DB optimization.** Intermediate status events (like "Thinking...", "Searching web...") are no longer persisted to the database during streaming; only final done=True statuses are saved, reducing unnecessary database writes. [#22085](https://github.com/open-webui/open-webui/pull/22085)
- 🔄 **Socket event type optimization.** Event type matching in the socket handler is now optimized using if/elif instead of if/if/if, avoiding redundant string comparisons after a match. [Commit](https://github.com/open-webui/open-webui/commit/ff86283be0479ccb86b639926b2b67ccbbe78746)
