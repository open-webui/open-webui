...
- 👥 Group sharing permissions now offer three granular options instead of a simple on/off toggle, allowing administrators to choose between "No one", "Members", or "Anyone" for who can share content to each group, with the default changed from member-only sharing to "Anyone", meaning users with sharing permissions can now share to any group regardless of membership unless administrators explicitly restrict it. [Commit](https://github.com/open-webui/open-webui/commit/ca514cd3eda2524b8da472ef17c0ccb216bac2e8)
