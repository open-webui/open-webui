...
usage() {
    ...
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --webui[port=3000] --ollama-data[folder=./data/ollama-data]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --webui[port=3000] --ollama-data[folder=./data/ollama-data] --webui-data[folder=./data/webui-data] --build"
    echo ""
...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        --open-webui-data*) # Used open-webui-data instead of webui-data because of the conflict with the webui flag catching webui-data
            value=$(extract_value "$key")
            webui_data_dir=${value:-"./webui-data"}
            ;;
        --drop)
            kill_compose=true
            ;;
...
if [[ $kill_compose == true ]]; then
    ...
    if [[ -n $webui_data_dir ]]; then
        ...
        DEFAULT_COMPOSE_COMMAND+=" -f docker-compose.webuidata.yaml"
        export OPEN_WEBUI_DATA_DIR=$webui_data_dir # Set OPEN_WEBUI_DATA_DIR environment variable
    fi
