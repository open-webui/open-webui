...
export const getDefaultRagStoreConfig = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/default_rag_store`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const setDefaultRagStoreConfig = async (token: string, storeName: string | null) => {
	let error = null;
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/default_rag_store`, {
		...
		body: JSON.stringify({
			DEFAULT_RAG_STORE_NAME: storeName
		})
	})
		.then(async (res) => {
			if (!res.ok) throw await res.json();
...
export const getLangfuseHost = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/langfuse_host`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
	...
	return res as { host: string | null; enabled: boolean };
};
