...
export const getModels = async (token: string = ''): Promise<AvailableModelsResponse> => {
    ...
    const res = await fetch(`${AUDIO_API_BASE_URL}/models`, {
        ...
        .catch((err) => {
            error = err.detail;
            console.log(err);
            return { models: [] }; // Return default structure on error
        });

    if (error) {
        // Consider removing throw if catch already handles return
        throw error;
    }

    return res ?? { models: [] }; // Ensure return structure is consistent
};
...
export const getVoices = async (token: string = '') => {
    ...
    const res = await fetch(`${AUDIO_API_BASE_URL}/voices`, {
        ...
        .catch((err) => {
            error = err.detail;
            console.log(err);
            return { voices: [] }; // Return default structure on error
        });
    ...
    return res ?? { voices: [] }; // Ensure return structure is consistent
};
