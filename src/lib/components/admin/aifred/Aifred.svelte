<script>
	...
	import OrgRescMgmt from './org-resrc-mgmt/OrgRescMgmt.svelte';
	import RagDataCreateMgmt from './rag-data-create-mgmt/RagDataCreateMgmt.svelte';

	const i18n = getContext('i18n');

	let selectedTab = 'orgResrcMgmt';
	let loaded = false;
...
<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-4">
	...
	<div class="flex-1 mt-1 lg:mt-0 overflow-y-scroll">
		{#if selectedTab === 'orgResrcMgmt'}
			<OrgRescMgmt />
		{:else if selectedTab === 'ragDataCreateMgmt'}
			<RagDataCreateMgmt />
		{/if}
	</div>
</div>
