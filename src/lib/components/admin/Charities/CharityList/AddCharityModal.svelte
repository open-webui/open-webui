<script lang="ts">
	...
	import { createCharity } from '$lib/apis/charities';
	...
	const submitHandler = async () => {
		...
		const res = await createCharity(localStorage.token, _charity).catch((error) => {
			toast.error(`${error}`);
		});
...
<Modal size="sm" bind:show>
	<div>
		<div class=" flex justify-between dark:text-gray-300 px-5 pt-4 pb-2">
			<div class=" text-lg font-medium self-center">{$i18n.t('Add Charity')}</div>
			<button
				class="self-center"
				on:click={() => {
					show = false;
