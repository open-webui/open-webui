<script lang="ts">
	...
	const runChatCleanup = async () => {
		cleanupLoading = true;
		try {
			// Determine cleanup parameters based on whether chat lifetime is enabled
			const cleanupParams = config.enabled
				? {
						// Chat lifetime is enabled - use configured settings
						max_age_days: config.days,
						preserve_pinned: config.preserve_pinned,
						preserve_archived: config.preserve_archived
					}
				: {
						// Chat lifetime is disabled - server will clean up ALL chats immediately
						// (parameters are still sent but server will override behavior)
						max_age_days: 0,
						preserve_pinned: config.preserve_pinned,
						preserve_archived: config.preserve_archived
					};

			const result = await triggerChatCleanup(localStorage.token, cleanupParams);

			if (result) {
				const chatsDeleted = result.cleanup_result?.chats_deleted || 0;
				const filesDeleted = result.cleanup_result?.files_cleaned || 0;
				const collectionsDeleted = result.cleanup_result?.collections_cleaned || 0;

				if (chatsDeleted > 0 || filesDeleted > 0 || collectionsDeleted > 0) {
					if (config.enabled) {
						toast.success(
							$i18n.t(
								'Cleanup completed: {{chatsDeleted}} expired chats deleted, {{filesDeleted}} files cleaned, {{collectionsDeleted}} collections removed',
								{
									chatsDeleted,
									filesDeleted,
									collectionsDeleted
						...
						toast.success(
							$i18n.t(
								'Cleanup completed: {{chatsDeleted}} chats deleted (all chats), {{filesDeleted}} files cleaned, {{collectionsDeleted}} collections removed',
								{
									chatsDeleted,
									filesDeleted,
									collectionsDeleted
					...
					if (config.enabled) {
						toast.success($i18n.t('Chat cleanup completed - no expired chats found'));
					} else {
						toast.success($i18n.t('Chat cleanup completed - no chats found for cleanup'));
					}
				}
				...
				toast.success($i18n.t('Chat cleanup completed successfully'));
			}
		} catch (error) {
			toast.error($i18n.t('Chat cleanup failed'));
