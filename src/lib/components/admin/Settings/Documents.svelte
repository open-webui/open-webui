							...
							<div class=" flex gap-1.5 w-full">
								<div class="w-full">
									<div class="self-center text-xs font-medium min-w-fit mb-1">
										<Tooltip
											placement="top-start"
											content={$i18n.t('Chunks smaller than this threshold will be merged with neighboring chunks when possible. Set to 0 to disable merging.')}
										>
											{$i18n.t('Chunk Merge Threshold')}
										</Tooltip>
									</div>
									<div class="self-center">
										<input
											class="w-full rounded-lg py-1.5 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
											type="number"
											placeholder={$i18n.t('Enter Threshold')}
											bind:value={RAGConfig.CHUNK_MERGE_THRESHOLD}
											autocomplete="off"
											min="0"
										/>
									</div>
