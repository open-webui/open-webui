<script lang="ts">
	...
	let taskConfig: {
		...
		VOICE_MODE_PROMPT_TEMPLATE: string;
		CUSTOM_FOLLOW_UP_SUGGESTIONS: Array<{ id: string; suggestion: string; context: string }>;
		USE_CUSTOM_FOLLOW_UP: boolean;
	} = {
		TASK_MODEL: '',
		TASK_MODEL_EXTERNAL: '',
	...
	const addFollowUp = () => {
		if (taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS.length < 20) {
			taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS = [
				...taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS,
				{ id: uuidv4(), suggestion: '', context: '' }
			];
		}
	};

	const removeFollowUp = (index) => {
		taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS.splice(index, 1);
		taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS = taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS;
	};
	...
	const updateInterfaceHandler = async () => {
		...
		try {
			...
			taskConfig.USE_CUSTOM_FOLLOW_UP = useCustomFollowUp;
			
			if (useCustomFollowUp) {
				const incompleteFollowUp = taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS.find(
			...
			toast.error($i18n.t('Failed to save settings. Please try again.'));
		} finally {
			loading = false;
		}
	};
	...
	const init = async () => {
		...
		if (!taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS) {
			taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS = [];
		}

		// Initialize USE_CUSTOM_FOLLOW_UP if not present
		if (taskConfig.USE_CUSTOM_FOLLOW_UP === undefined) {
			taskConfig.USE_CUSTOM_FOLLOW_UP = taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS.length > 0;
		}
		
		useCustomFollowUp = taskConfig.USE_CUSTOM_FOLLOW_UP;
            ...
            <div class="mb-3.5">
                ...
                {#if taskConfig.ENABLE_FOLLOW_UP_GENERATION}
                    <div class="mb-2.5">
                        ...
                        <Tooltip
                            content={useCustomFollowUp 
                                ? $i18n.t('This prompt will only be used if there is no match between custom follow-up suggestions and the chat. Leave empty to use the default prompt.')
                    ...
                    <div class="mb-2.5 flex w-full items-center justify-between">
                        <div class=" self-center text-xs font-medium">
                            {$i18n.t('Use Custom Follow-up Suggestion')}
                        </div>

                        <Switch bind:state={useCustomFollowUp} />
                    </div>

                    {#if useCustomFollowUp}
                        <div class="mb-2.5 space-y-2">
                            {#each taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS as item, i (item.id)}
                                <div class="flex items-start gap-2">
                                    <div class="grid grid-cols-2 gap-2 flex-1">
                                        <div>
                                            <div class=" mb-1 text-xs font-medium">
                                                {$i18n.t('Custom Follow-up Suggestion')} #{i + 1}
                                            </div>
                                            <Tooltip
                                                content={$i18n.t(
                                            ...
                                            >
                                                <Textarea
                                                    bind:value={item.suggestion}
                                                    placeholder={$i18n.t('Enter a custom follow-up suggestion')}
                                                    rows={2}
                                                />
                                            </Tooltip>
                                        </div>
                                        <div>
                                            <div class=" mb-1 text-xs font-medium">{$i18n.t('Context Description')}</div>
                                            <Tooltip
                                                content={$i18n.t(
                                                    'Describe when this follow-up should be suggested. The system will use semantic analysis to match similar conversations.'
                                                )}
                                                placement="top-start"
                                            >
                                                <Textarea
                                                    bind:value={item.context}
                                                    placeholder={$i18n.t('Describe the context for this suggestion')}
                                                    rows={2}
                                                />
                                            </Tooltip>
                                        </div>
                                    ...
                                    <button
                                        class="mt-7 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
                                        type="button"
                                        on:click={() => removeFollowUp(i)}
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 20 20"
                            ...
                            {#if taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS.length < 20 && (taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS.length === 0 || taskConfig.CUSTOM_FOLLOW_UP_SUGGESTIONS.at(-1)?.suggestion !== '')}
                                <div class="flex justify-start">
                                    <button
                                        type="button"
                                        class="flex items-center bg-green-600 hover:bg-green-700 text-white text-xs px-3 py-1.5 rounded-lg transition"
                                    ...
                                    >
                                        ...
                                        <span>{$i18n.t('Add Follow-up')}</span>
                                    </button>
                                </div>
                            {/if}
                        </div>
