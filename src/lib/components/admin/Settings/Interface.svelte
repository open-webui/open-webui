			...
			<div class="mb-3.5">
				...
				<div class=" mb-2.5 flex w-full gap-2">
					<div class="flex-1">
						...
						>
							<option selected value="">{$i18n.t('Current Model')}</option>
							{#each $models.filter((m) => m.owned_by === 'ollama') as model}
								<option class="bg-gray-100 dark:bg-gray-700" value={model.id}>
									{model.name}
								</option>
							{/each}
						</select>
						...
						{#each banners as banner, bannerIdx}
							<div class=" flex justify-between">
								...
								>
									...
									>
										{#if banner.type == ''}
											<option class="text-gray-900" disabled selected value=""
												>{$i18n.t('Type')}</option
											>
										{/if}
										<option class="text-gray-900" value="info">{$i18n.t('Info')}</option>
										<option class="text-gray-900" value="warning">{$i18n.t('Warning')}</option>
										<option class="text-gray-900" value="error">{$i18n.t('Error')}</option>
