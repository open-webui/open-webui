<script lang="ts">
	...
	let taskConfig = {
		...
		TASK_MODEL_EXTERNAL: '',
		GROUNDING_MODEL: '',
		GROUNDING_MODEL_WIDTH: 1000,
		GROUNDING_MODEL_HEIGHT: 1000,
	...
	>
		<div class="  overflow-y-scroll scrollbar-hidden h-full pr-1.5">
			<div class="mb-3.5">
				...
				<div class=" mb-2 font-medium flex items-center">
					<div class=" text-xs mr-1">{$i18n.t('Grounding Model')}</div>
					<Tooltip
						content={$i18n.t(
							'A vision model used to locate UI elements on the desktop. Called by the harness model to find precise click targets.'
						)}
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
				...
				<div class=" mb-2.5 flex w-full gap-2">
					<div class="flex-1">
						<select
							class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
							bind:value={taskConfig.GROUNDING_MODEL}
							placeholder={$i18n.t('Select a model')}
							on:change={() => {
								if (taskConfig.GROUNDING_MODEL) {
									const model = models.find((m) => m.id === taskConfig.GROUNDING_MODEL);
									if (model) {
										taskConfig.GROUNDING_MODEL = model.id;
									} else {
										taskConfig.GROUNDING_MODEL = '';
									}
								}
							}}
						>
							<option value="" selected>{$i18n.t('Disabled')}</option>
							{#each models as model}
								<option value={model.id} class="bg-gray-100 dark:bg-gray-700">
									{model.name}
									{model?.connection_type === 'local' ? `(${$i18n.t('Local')})` : ''}
				...
				<div class="mb-2.5 flex w-full gap-2">
					<div class="flex-1">
						<div class=" mb-1 text-xs font-medium">{$i18n.t('Grounding Coordinate Width')}</div>
						<input
							type="number"
							class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
							bind:value={taskConfig.GROUNDING_MODEL_WIDTH}
							min="1"
						/>
					</div>
					<div class="flex-1">
						<div class=" mb-1 text-xs font-medium">{$i18n.t('Grounding Coordinate Height')}</div>
						<input
							type="number"
							class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
							bind:value={taskConfig.GROUNDING_MODEL_HEIGHT}
							min="1"
						/>
					</div>
				</div>
