<script lang="ts">
	...
	const saveServerHandler = async () => {
		try {
			...
			const args = serverFormArgsText.trim()
				? serverFormArgsText
						.split('\n')
						.map((arg) => arg.trim())
						.filter((arg) => arg)
			...
			if (res) {
				toast.success(
					editingServer
						? $i18n.t('Server updated successfully')
						: $i18n.t('Server created successfully')
				);
				closeServerModal();
	...
	<div class="space-y-3 overflow-y-scroll scrollbar-hidden h-full">
		<div class="flex flex-col gap-1 mt-1.5 mb-2">
			...
			<div class="text-xs text-gray-600 dark:text-gray-500">
				{$i18n.t(
					'Configure connections to external MCP servers to extend model capabilities with additional tools and context.'
				)}
			</div>
		</div>
		...
		{#if ENABLE_MCP_API}
			<div class="my-2 mb-5">
				...
				<div class="mb-6">
							...
							>
								<div class="flex flex-1 text-left space-x-3.5 w-full">
									...
									<div class="flex-1 self-center">
										...
										<div class="text-xs text-gray-700 dark:text-gray-400 line-clamp-1">
											{server.tools_count}
											{server.tools_count === 1 ? $i18n.t('tool') : $i18n.t('tools')}
										</div>
									</div>
								</div>
								...
								<div class="flex self-center space-x-2">
									<div class="flex items-center gap-2">
										<span
											class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {server.status ===
											'running'
					...
					{:else}
						<div
							class="text-xs text-gray-700 dark:text-gray-400 p-3 border border-dashed dark:border-gray-700 rounded-lg text-center"
						>
						...
						{#each externalServers as server}
							...
							>
								<div class="flex flex-1 text-left space-x-3.5 w-full">
									<div class="self-center w-8">
										<div
											class="rounded-full object-cover {server.runtime_status === 'running'
												? ''
												: 'opacity-50 dark:opacity-50'}"
										>
											<svg
									...
									<div class="flex-1 self-center">
										...
										<div class="text-xs text-gray-700 dark:text-gray-400 line-clamp-1">
											...
											{(server.tools_count || 0) === 1 ? $i18n.t('tool') : $i18n.t('tools')}
										</div>
									</div>
								</div>
								...
								<div class="flex self-center space-x-2">
									<div class="flex items-center gap-2">
										<span
											class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {server.runtime_status ===
											'running'
												? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300'
												: 'bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300'}"
										>
							...
							>
								...
								{:else if tool.is_builtin === true}
									...
									<div class="flex items-center gap-1 mt-2 ml-6">
										...
										<span class="text-xs text-gray-600"
											>{$i18n.t('Built-in Server')}: {tool.mcp_server_name
												? formatServerName(tool.mcp_server_name)
												: $i18n.t('Unknown')}</span
										>
									</div>
								{:else if tool.mcp_server_name}
									...
									<div class="flex items-center gap-1 mt-2 ml-6">
										...
										<span class="text-xs text-gray-600"
											>{$i18n.t('External Server')}: {tool.mcp_server_name}</span
										>
									</div>
								{/if}
								{#if tool.inputSchema}
									<details class="mt-2 ml-6">
										<summary
											class="text-xs cursor-pointer text-gray-600 hover:text-gray-500 flex items-center gap-1"
										>
											<svg
												xmlns="http://www.w3.org/2000/svg"
												viewBox="0 0 20 20"
...
{JSON.stringify(tool.inputSchema, null, 2)}
				...
				{:else if !mcpToolsLoading}
					<div
						class="flex items-center gap-2 text-xs text-gray-600 p-4 border border-dashed dark:border-gray-600 rounded-lg"
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 20 20"
						...
						{$i18n.t(
							'No MCP tools available. Configure and enable MCP servers to see available tools.'
						)}
					</div>
				{/if}
		...
		<div class="px-6 py-4">
			...
			<div class="space-y-4 max-h-96 overflow-y-auto">
				<form on:submit|preventDefault={saveServerHandler} class="space-y-4">
					...
					<div class="space-y-3">
						<h3 class="text-sm font-medium text-gray-900 dark:text-gray-100">
							{$i18n.t('Transport Configuration')}
						</h3>
					...
					<div class="space-y-3">
						<h3 class="text-sm font-medium text-gray-900 dark:text-gray-100">
							{$i18n.t('Server Options')}
						</h3>

						<div class="flex items-center justify-between">
							<div>
								<div class="text-sm font-medium text-gray-700 dark:text-gray-300">
									{$i18n.t('Start server automatically')}
								</div>
								<div class="text-xs text-gray-500">
									{$i18n.t('Start this server when the application starts')}
								</div>
							</div>
