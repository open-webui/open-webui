					...
					<div>
						...
						<div class="flex w-full">
							<div class="flex-1">
								...
								>
									<option selected={TTS_VOICE !== ''} value="">{$i18n.t('Default')}</option>
									{#each voices as voice}
										<option
											class="bg-gray-100 dark:bg-gray-700"
											selected={TTS_VOICE === voice.voiceURI}
											value={voice.voiceURI}>{voice.name.replace('+', ', ')}</option
										>
									{/each}
								</select>
							</div>
								...
								<div class="flex-1">
									...
									<datalist id="tts-model-list">
										{#each models as model}
											<option class="bg-gray-50 dark:bg-gray-700" value={model.id}></option>
										{/each}
									</datalist>
								</div>
							</div>
