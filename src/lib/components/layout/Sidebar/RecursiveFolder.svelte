...
const deleteHandler = async () => {
	const res = await deleteFolderById(localStorage.token, folderId).catch(
		(error) => {
			toast.error(error);
			return null;
		},
...
const nameUpdateHandler = async () => {
	...
	const res = await updateFolderNameById(
		localStorage.token,
		folderId,
		name,
	).catch((error) => {
...
const isExpandedUpdateHandler = async () => {
	const res = await updateFolderIsExpandedById(
		localStorage.token,
		folderId,
		open,
	).catch((error) => {
...
const exportHandler = async () => {
	const chats = await getChatsByFolderId(localStorage.token, folderId).catch(
		(error) => {
			toast.error(error);
			return null;
		},
