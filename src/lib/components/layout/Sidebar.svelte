...
{#if $showSidebar}
	...
	>
		...
		>
			...
			<!-- Quick Menu Section (Admin Only) -->
			{#if $user?.role === 'admin'}
				<div class="sticky bottom-20 z-10 sidebar mt-2  mx-2 rounded-t-2xl">
					<div class="bg-gray-50 dark:bg-gray-850 rounded-lg overflow-hidden">
						<button
							class="w-full flex items-center justify-between p-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition"
							on:click={() => showQuickMenu = !showQuickMenu}
						>
							...
							<svg
								...
								stroke-width="1.5"
								stroke="currentColor"
								class="size-4 text-gray-500 dark:text-gray-400 transition-transform duration-200 {showQuickMenu ? 'rotate-180' : ''}"
							>
								<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
							</svg>
						</button>
						{#if showQuickMenu}
							<div class="grid grid-cols-3 gap-2 p-3 pt-0" transition:slide={{ duration: 200 }}>
							<!-- 온라인 지식기반 -->
							<a
								href="/workspace/online-knowledge"
								class="flex flex-col items-center justify-center p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition group"
								on:click={() => {
									if ($mobile) {
										showSidebar.set(false);
									}
							...
							>
								<svg
									...
									stroke-width="1.5"
									stroke="currentColor"
									class="size-5 mb-1 text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white"
								>
									<path
										stroke-linecap="round"
										stroke-linejoin="round"
								...
								<span class="text-xs text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">지식기반</span>
							</a>
							...
							>
								...
								<span class="text-xs text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">챕터</span>
							</a>

							<!-- Message Tags -->
							<a
								href="/admin/settings/message-tags"
								class="flex flex-col items-center justify-center p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition group"
								on:click={() => {
									if ($mobile) {
										showSidebar.set(false);
							...
							>
								...
								<span class="text-xs text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">태그</span>
							</a>

							<!-- Personas -->
							<a
								href="/admin/settings/personas"
								class="flex flex-col items-center justify-center p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition group"
								on:click={() => {
									if ($mobile) {
										showSidebar.set(false);
							...
							>
								...
								<span class="text-xs text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">페르소나</span>
							</a>
							...
							>
								...
								<span class="text-xs text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">모델</span>
							</a>
							...
							>
								...
								<span class="text-xs text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">앱 설정</span>
							</a>
							</div>
						{/if}
					</div>
			...
			<div class="sticky bottom-0 z-10 sidebar flex flex-row justify-between items-center p-5 gap-5 h-20">
				...
				<div class="flex flex-row items-center gap-3 flex-1 hover:bg-gray-100/50 dark:hover:bg-gray-850/50 transition rounded-xl p-1 -m-1">
					{#if $user !== undefined && $user !== null}
						<UserMenu
							role={$user?.role}
							on:show={(e) => {
						...
						>
							<div class="flex flex-row items-center gap-3 rounded-xl   py-1 px-1 -mx-1">
