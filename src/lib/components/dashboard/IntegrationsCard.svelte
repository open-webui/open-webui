<script lang="ts">
	...
	async function connectGoogle() {
		...
		connecting = true;
		try {
			const res = await fetch(`${WEBUI_BASE_URL}/api/v1/clapnclaw/integrations/connect-url/google`, {
		...
		window.location.href = 'mailto:hola@clapnclaw.com?subject=Google%20Workspace%20Integration';
		connecting = false;
	}
...
<div class="rounded-2xl px-5 py-4 border bg-white dark:bg-gray-800 border-black/[.07] dark:border-white/[.07]">
	...
	<div class="flex flex-wrap gap-3">
		...
		>
			...
			{:else}
				<button
					class="text-[10px] font-mono font-semibold ml-2 px-2 py-1 rounded-full cursor-pointer hover:opacity-80 transition-opacity disabled:opacity-50"
