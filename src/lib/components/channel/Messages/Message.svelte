...
{#if message}
  ...
  >
    ...
    >
      <div class={`shrink-0 ${($settings?.chatDirection ?? 'LTR') === 'LTR' ? 'mr-3' : 'ml-3'} w-9`}>
        {#if showUserProfile}
      ...
      <div class="flex-auto w-0 pl-1">
        ...
        {:else}
          <div class=" min-w-full markdown-prose">
            ...
            />{#if message.created_at !== message.updated_at}<span class="text-gray-500 text-[10px]">(edited)</span>{/if}
          </div>
            ...
            <div class="flex items-center gap-1.5 -mt-0.5 mb-1.5">
              ...
              >
                <span class="font-medium mr-1">
                  {$i18n.t('{{COUNT}} Replies', { COUNT: message.reply_count })}</span><span>
                  {' - '}{$i18n.t('Last reply')}
