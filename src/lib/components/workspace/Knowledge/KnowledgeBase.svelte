							...
							<div class="flex flex-col justify-start h-full max-h-full">
								<div class=" flex flex-col w-full h-full max-h-full">
									...
									{#if selectedFile?.meta}
										{@const systemKeys = ['name', 'content_type', 'size', 'collection_name', 'data']}
										{@const metaEntries = Object.entries(selectedFile.meta).filter(
											([key, value]) => !systemKeys.includes(key) && value != null && value !== ''
										)}
										{#if metaEntries.length > 0}
											<div class="shrink-0 flex flex-wrap gap-x-3 gap-y-1 px-3 pb-2 text-xs text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-850">
												{#each metaEntries as [key, value]}
													<span>
														<span class="font-medium text-gray-600 dark:text-gray-300">{key.replace(/_/g, ' ')}:</span>
