	...
	const getAvailableSettings = () => {
		return allSettings.filter((tab) => {
			...
			if (tab.id === 'tools') {
				return (
					$user?.role === 'admin' ||
					(['user', 'knowledge'].includes($user?.role) &&
			...
			>
				...
				{#if filteredSettings.length > 0}
					{#each filteredSettings as tabId (tabId)}
						...
						{#if $user?.role === 'admin' || (['user', 'knowledge'].includes($user?.role) && $config?.features?.enable_direct_connections)}
								<button
									role="tab"
							...
							{/if}
						{:else if tabId === 'tools'}
						{#if $user?.role === 'admin' || (['user', 'knowledge'].includes($user?.role) && $user?.permissions?.features?.direct_tool_servers)}
								<button
									role="tab"
