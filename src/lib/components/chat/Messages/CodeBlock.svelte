<script lang="ts">
  ...
  const executePython = async (code) => {
    ...
    if ($config?.code?.engine === 'jupyter') {
      const output = await executeCode(localStorage.token, code).catch(
        (error) => {
          toast.error(`${error}`);
          return null;
        }
  ...
  const updateButtonPosition = () => {
    ...
    if (collapsed) {
      ...
      const maxExpandedOffset = Math.max(0, codeBlockRect.height - buttonsElement.offsetHeight - 20);
  ...
  // Removed editor ready handling - no longer needed
...
<div bind:this={codeBlockElement}>
  ...
  >
    ...
    {:else}
      <div
        class="text-text-300 absolute pl-4 py-1.5 text-xs font-medium dark:text-white"
      >
        {lang}
      </div>
