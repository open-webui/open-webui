<script lang="ts">
	...
	import SpeakerXMark from '$lib/components/icons/SpeakerXMark.svelte';
	import Photo from '$lib/components/icons/Photo.svelte';
	import Info from '$lib/components/icons/Info.svelte';
	import HandThumbUp from '$lib/components/icons/HandThumbUp.svelte';
	import HandThumbDown from '$lib/components/icons/HandThumbDown.svelte';
								...
								{#if !readOnly}
									{#if !$temporaryChatEnabled && ($config?.features.enable_message_rating ?? true)}
										...
										<Tooltip content={$i18n.t('Bad Response')} placement="bottom">
											...
											>
												<HandThumbDown className="w-4 h-4" />
											</button>
										</Tooltip>
									{/if}
