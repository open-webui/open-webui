							...
							<div class="w-full flex flex-col relative" id="response-content-container">
									...
									<div class="flex flex-col gap-2 py-3 px-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-200 dark:border-yellow-800">
										...
										<div class="w-full bg-yellow-200 dark:bg-yellow-800 rounded-full h-1">
											<div
												...
												style="width: {((message.retrying.attempt * 2 - message.retrying.countdown) / (message.retrying.attempt * 2)) * 100}%"
											/>
										</div>
									</div>
								{/if}
								...
								{#if message?.error}
									<Error
										...
										onRetryWithoutProvider={message?.providerFailed ? () => retryWithoutProviderRestrictions(message) : null}
									/>
								{/if}
