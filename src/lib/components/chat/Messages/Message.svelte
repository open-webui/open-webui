<script lang="ts">
	...
	function parseBase64JSON(base64String: string) {
		try {
	...
	function cleanMessageContent(content: string): string {
		const combinedRegex =
			/:::customImage\s*({.*?})\s*:::|:::customText\s*({[\s\S]*?})\s*:::|:::customURL\s*({.*?})\s*:::/g;

		return content.replace(combinedRegex, (fullMatch, imageJSON, textJSON, urlJSON) => {
			try {
				if (textJSON) {
					const { base64 } = JSON.parse(textJSON);
					if (base64) {
						const json = parseBase64JSON(base64);
						// 👉 SI ARRAY, prends le premier élément
						if (Array.isArray(json)) {
							return json
								.map(
									(item) =>
										`<div class="custom-text">${item?.page_content || '[Texte inconnu]'}</div>`
								)
								.join('');
						}
						return `<div class="custom-text">${json?.page_content || '[Texte inconnu]'}</div>`;
					}
				}
				if (imageJSON) {
					const json = JSON.parse(imageJSON);
					return `<div class="custom-image">[Image custom: ${json?.src || 'source inconnue'}]</div>`;
				}
				if (urlJSON) {
					const { url } = JSON.parse(urlJSON);
					if (url) return `<a href="${url}" target="_blank">Download ⭳</a>`;
				}
			} catch {
				return '';
			}
...
function extractAndCleanSources(content: string) {
	const sourcesRegex = /BEGIN_SOURCES\s*({[\s\S]*})\s*END_OF_MESSAGE/;
	const match = content.match(sourcesRegex);
	let sources: any[] = [];
	let cleanedContent = content;

	function tryParseBase64Field(field) {
		try {
			if (!field) return null;
			const binaryString = atob(field);
			const bytes = new Uint8Array(binaryString.length);
			for (let i = 0; i < binaryString.length; i++) {
				bytes[i] = binaryString.charCodeAt(i);
			}
	...
	if (match && match[1]) {
		try {
			...
			let parsed = [];
			if (json.base64) {
				parsed = tryParseBase64Field(json.base64);
			}
			// flatten array or single
			const sourceArr = Array.isArray(parsed) ? parsed : parsed ? [parsed] : [];
			// Pour chaque source, si le contenu de page est encore base64, décode-le
			sources = sourceArr.map((src, idx) => {
				// Certains dumps mal formés peuvent avoir page_content en base64 (rare), on gère
				let page_content = src.page_content;
				if (
					page_content &&
					/^[A-Za-z0-9+/=]+$/.test(page_content) &&
					page_content.length > 50 // pour éviter de décoder de petites chaînes normales
				) {
					const decoded = tryParseBase64Field(page_content);
					page_content = typeof decoded === "string" ? decoded : page_content;
				}
				return {
					filename: src.filename ?? `Source #${idx + 1}`,
					page: src.page,
					page_content: page_content ?? JSON.stringify(src, null, 2),
				};
			});
		} catch (e) {
			console.warn('Error parsing sources:', e);
		}
		cleanedContent = cleanedContent.replace(sourcesRegex, '').trim();
	}
	...
	$: cleanedContentAndSources = message?.content
		? extractAndCleanSources(message.content)
		: { cleanedContent: '', sources: [] };
</script>
			...
			<div class="flex-auto w-0 pl-1">
				...
				{:else}
					<div class="min-w-full markdown-prose">
						<Markdown source={cleanMessageContent(cleanedContentAndSources.cleanedContent)} />
						{#if message.created_at !== message.updated_at}
							<span class="text-gray-500 text-[10px]">(edited)</span>
						{/if}
					</div>
					{#if cleanedContentAndSources.sources.length > 0}
						<div
							class="mt-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700"
						>
							<div class="font-bold mb-1">Sources utilisées :</div>
							<ul class="text-xs">
								{#each cleanedContentAndSources.sources as source}
									<li class="mb-1">
