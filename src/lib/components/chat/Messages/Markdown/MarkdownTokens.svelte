...
{#each tokens as token, tokenIdx (tokenIdx)}
	...
	{:else if token.type === 'heading'}
		<svelte:element this={headerComponent(token.depth)} dir="auto">
			<MarkdownInlineTokens id={`${id}-${tokenIdx}-h`} {onSourceClick} tokens={token.tokens} />
		</svelte:element>
	...
	{:else if token.type === 'text'}
		{#if top}
			<p dir="auto">
				{#if token.tokens}
					<MarkdownInlineTokens id={`${id}-${tokenIdx}-t`} {onSourceClick} tokens={token.tokens} />
				{:else}
