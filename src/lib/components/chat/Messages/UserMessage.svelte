...
<div id="message-{message.id}" class=" flex w-full user-message" dir={$settings.chatDirection}>
			...
			{#if message.files}
				<div class="mt-2.5 mb-1 w-full flex flex-col justify-end overflow-x-auto gap-1 flex-wrap">
					{#each message.files as file}
						<div class:self-end={$settings?.chatBubble ?? true}>
							{#if file.type === 'image'}
								<Image imageClassName=" max-h-96 rounded-lg" src={file.url} />
							{:else}
								<FileItem
									name={file.name}
			...
			{#if message.content !== ''}
				...
				{:else}
					<div class="w-full">
						...
						<div
							class=" flex text-gray-600 dark:text-gray-500"
							class:justify-end={$settings?.chatBubble ?? true}
						>
							{#if !($settings?.chatBubble ?? true)}
