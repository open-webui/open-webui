...
>
	...
	<div slot="content">
		<DropdownMenu.Content
			class="w-full max-w-[220px] rounded-xl px-2 py-2 border border-gray-200 dark:border-gray-700 z-50 bg-white dark:bg-gray-850 shadow-lg"
			sideOffset={10}
			alignOffset={-8}
			side="top"
			align="start"
		...
		>
			{#if Object.keys(tools).length > 0}
				<div class="mb-2">
					...
					<div class="max-h-32 overflow-y-auto scrollbar-hidden space-y-0.5">
						{#each Object.keys(tools) as toolId}
							<button
								class="flex w-full justify-between gap-2 items-center px-3 py-2.5 text-sm font-medium cursor-pointer rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
								on:click={() => {
			...
			<div class="space-y-0.5">
				...
				>
					<DropdownMenu.Item
						class="flex gap-3 items-center px-3 py-2.5 text-sm font-medium cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-colors {!fileUploadEnabled
					...
					>
						<div class="text-gray-600 dark:text-gray-400">
							<CameraSolid className="size-4" />
						</div>
						<div class="line-clamp-1 text-gray-700 dark:text-gray-200">{$i18n.t('Capture')}</div>
					</DropdownMenu.Item>
				</Tooltip>
				...
				>
					...
					>
						...
						<div class="line-clamp-1 text-gray-700 dark:text-gray-200">{$i18n.t('Upload Files')}</div>
					</DropdownMenu.Item>
				</Tooltip>

				{#if $config?.features?.enable_google_drive_integration}
					<DropdownMenu.Item
						class="flex gap-3 items-center px-3 py-2.5 text-sm font-medium cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-colors"
						on:click={() => {
					...
					>
						...
						<div class="line-clamp-1 text-gray-700 dark:text-gray-200">{$i18n.t('Google Drive')}</div>
					</DropdownMenu.Item>
				{/if}
				...
				{#if $config?.features?.enable_onedrive_integration}
					...
					>
						<div class="shrink-0">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" class="w-4 h-4" fill="none">
								<mask
									id="mask0_87_7796"
						...
						<div class="line-clamp-1 text-gray-700 dark:text-gray-200">{$i18n.t('OneDrive')}</div>
					</DropdownMenu.Item>
				{/if}
			</div>
		</DropdownMenu.Content>
