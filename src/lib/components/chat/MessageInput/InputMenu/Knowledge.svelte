<script lang="ts">
	...
	import KnowledgeFileTree from './KnowledgeFileTree.svelte';
	...
	// Knowledge base whose files are currently expanded.
			...
			{#each items as item, idx (item.id)}
				...
				{#if selectedItem && selectedItem.id === item.id}
					<div class="pl-3 mb-1 flex flex-col gap-0.5">
						<KnowledgeFileTree knowledgeId={item.id} directoryId={null} {onSelect} />
					</div>
				{/if}
			{/each}
