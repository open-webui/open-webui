<script lang="ts">
	...
	// Determine if we should use collapsible tags at all
	$: useCollapsibleTags = uniqueTags.length > 0;

	// Get models that don't belong to any collapsible tag group (only compute if using collapsible tags)
	$: otherModels = useCollapsibleTags ? items.filter(item => {
		const tags = item.model?.info?.meta?.tags || [];
	...
	// Group items by their tags for nested display (only compute if using collapsible tags)
	$: groupedItems = !searchValue && useCollapsibleTags ? uniqueTags.reduce((acc, tag) => {
		...
		<slot>
			...
			<div class="px-3 my-2 max-h-64 overflow-y-auto scrollbar-hidden group">
				...
				{:else}
					...
					{/if}
					{:else}
						<!-- Flat list of all models when no collapsible tags are needed -->
						{#each items as item}
							<button
								aria-label="model-item"
								class="flex w-full text-left font-medium line-clamp-1 select-none items-center rounded-button py-2 pl-3 pr-1.5 text-sm text-gray-700 dark:text-gray-100 outline-hidden transition-all duration-75 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg cursor-pointer"
								...
								<div class="flex flex-col">
									{#if $mobile && (item?.model?.info?.meta?.tags ?? []).length > 0}
										<div class="flex gap-0.5 self-start h-full mb-1.5 -translate-x-1">
											{#each item.model?.info?.meta.tags as tag}
												<div
													class="flex items-center text-xs font-bold px-1 rounded-sm uppercase line-clamp-1 bg-gray-500/20 text-gray-700 dark:text-gray-200"
												>
													{tag.name}
												</div>
											{/each}
