...
<div class="space-y-4 text-sm pb-safe-bottom">
	...
	<div class="space-y-3">
		<div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3">
			...
			>
				<div class="flex items-center justify-between">
					...
					<button
						class="px-4 py-1.5 text-sm rounded-md bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors"
						on:click={() => {
		...
		<div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3">
			<Tooltip
				content={$i18n.t(
					'Default mode works with a wider range of models by calling tools once before execution. Native mode leverages the model built-in tool-calling capabilities, but requires the model to inherently support this feature.'
				)}
				placement="top-start"
				className="inline-tooltip"
			>
	...
	<div class="space-y-3">
		...
		<div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3">
			...
			{#if (params?.seed ?? null) !== null}
				<input
					class="w-full rounded-md py-2 px-3 text-sm bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500 outline-none"
					type="number"
			...
			{#if (params?.temperature ?? null) !== null}
				<div class="flex items-center gap-3">
					<input
						...
						class="flex-1 h-2 rounded-lg appearance-none cursor-pointer bg-gray-200 dark:bg-gray-700"
					/>
					<input
						bind:value={params.temperature}
						type="number"
						class="w-16 px-2 py-1 text-center rounded-md bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600"
						min="0"
						max="2"
						step="any"
					/>
			...
			{#if (params?.num_keep ?? null) !== null}
				<div class="flex items-center gap-3">
					...
					<input
						bind:value={params.num_keep}
						type="number"
						class="w-20 px-2 py-1 text-center rounded-md bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600"
						min="-1"
						step="1"
					/>
				</div>
