<script lang="ts">
	...
	import {
		...
		uploadBilibiliTranscriptionToVectorDB
	} from '$lib/apis/rag';
	...
	const uploadBilibiliTranscription = async (url) => {
		console.log(url);
		...
		try {
			files = [...files, doc];
			const res = await uploadBilibiliTranscriptionToVectorDB(localStorage.token, url);
				...
				<div class="w-full relative">
					...
					{:else if prompt.charAt(0) === '#'}
						<Documents
							...
							on:bilibili={(e) => {
								console.log(e);
								uploadBilibiliTranscription(e.detail);
							}}
							on:youtube={(e) => {
								console.log(e);
