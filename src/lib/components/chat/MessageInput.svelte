	...
	const uploadFileHandler = async (file, fullContext: boolean = false) => {
		...
		if (!$temporaryChatEnabled) {
			try {
                ...
                const uploadedFile = await uploadFile(localStorage.token, file, metadata, { process: false });
                        ...
                        try {
                            ...
                            if (res.ok) {
                                ...
                                if (processing) {
                                    ...
                                    fileItem.progress = typeof processing.progress === 'number' ? processing.progress : fileItem.progress ?? 0;
                                    files = files;
                                    if (processing.status === 'done' || (typeof processing.progress === 'number' && processing.progress >= 100)) {
