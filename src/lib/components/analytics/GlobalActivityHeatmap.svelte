<script lang="ts">
	...
	import { getGlobalHeatmap, formatTokenCount, type HeatmapDataPoint } from '$lib/apis/analytics';
	...
	async function loadHeatmapData() {
		...
		try {
			...
			const response = await getGlobalHeatmap(token, year);
			if (response) {
				...
				error = 'Failed to load global heatmap data';
			}
		} catch (e) {
			error = 'Error loading global heatmap data';
			console.error(e);
		}
								...
								<Tooltip
									content={`
										<div class="text-xs">
											...
											<div class="text-gray-300">${day.tokens > 0 ? formatTokenCount(day.tokens) + ' tokens (site-wide)' : 'No activity'}</div>
										</div>
									`}
									placement="top"
								>
