...
onMount(async () => {
if (!element) {
console.error('RichTextInput: element is not available onMount');
return;
}
console.log(value);
...
if (!raw) {
async function tryParse(inputValue: string, attempts = 3, interval = 100): Promise<string> {
try {
// Try parsing the value
return marked.parse(inputValue.replaceAll(`\n<br/>`, `<br/>`), {
breaks: false
});
} catch (error) {
...
return tryParse(inputValue, attempts - 1, interval); // Recursive call
}
}
...
.getHTML()
.replace(/<p><\/p>/g, '<br/>')
.replace(/ {2,}/g, (m: string) => m.replace(/ /g, ''))
)
.replace(/<!-- -->/g, ' ');
...
attributes: { id: id },
handleDOMEvents: {
compositionstart: (view: EditorView, event: CompositionEvent) => {
...
.replace(/<!-- -->/g, ' ')
  ...
  if (raw) {
    ...
    if (preserveBreaks) {
      newContentToSet = value;
    } else {
      newContentToSet = marked.parse(value.replaceAll(`\n<br/>`, `<br/>`), {
        breaks: false
      });
    }
