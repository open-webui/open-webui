	...
	{#if loaded}
		...
		{#if collapsible}
			...
			>
				...
				<div
					id="{id}-folder-button"
					class=" w-full group relative flex items-center justify-between transition {buttonClassName}"
				>
					<button class="w-full py-1 pl-2.5 relative flex items-center text-[11px] font-semibold uppercase tracking-wide">
						<div class="absolute left-0 top-1/2 -translate-y-1/2 p-[1px] text-gray-400 dark:text-gray-600 invisible group-hover:visible">
							{#if open}
				...
				<div class="translate-y-[0.5px] flex items-center gap-1">
					{name}
						{#if badge > 0 && !open}
							<span class="flex-shrink-0 min-w-[1.1rem] h-[1.1rem] px-1 flex items-center justify-center rounded-full bg-amber-500 text-white text-[10px] font-bold leading-none">
								{badge}
							</span>
						{/if}
					</div>
