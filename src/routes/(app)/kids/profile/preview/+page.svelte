<script lang="ts">
	...
	function parseChildCharacteristics(characteristics: string) {
		...
		if (
			characteristics.includes('Selected characteristics:') &&
			characteristics.includes('Additional characteristics:')
		) {
			...
			if (selectedStart > 0) {
				...
				if (traitMatch) {
					...
					result.selectedCharacteristics = traitMatch[2]
						.split(',')
						.map((char) => char.trim())
						.filter((char) => char);
		...
		<div class="max-w-4xl mx-auto px-4 py-8">
			...
			<div class="mb-8">
				...
				<div class="flex items-center justify-between">
					<h1 class="text-3xl font-bold text-gray-900 dark:text-white">Child Profile Preview</h1>
						...
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							{#each childProfiles as child, index}
								<button
									...
									class="p-4 rounded-lg border-2 transition-all duration-200 text-left {selectedChildIndex ===
									index
									...
									<div class="flex items-center space-x-3">
										...
										<div class="flex-1">
											...
											<p class="text-sm text-gray-500 dark:text-gray-400">
												{child.child_age || 'Age not set'} • {child.child_gender ||
													'Gender not set'}
											</p>
										</div>
									</div>
				...
				{#if currentChild}
					<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
						<div class="flex items-center space-x-6 mb-8">
							<div
								class="w-20 h-20 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center shadow-lg"
							>
							...
							<div>
								...
								<div class="flex items-center space-x-4">
									...
									{#if currentChild.child_gender}
										<span
											class="bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-200 px-3 py-1 rounded-full text-sm font-medium"
										>
								...
								<div class="space-y-3">
									...
									>
										...
										<span class="text-gray-900 dark:text-white"
											>{currentChild.name || 'Not set'}</span
										>
									</div>
									<div
										class="flex justify-between py-2 border-b border-gray-200 dark:border-gray-700"
									>
										...
										<span class="text-gray-900 dark:text-white"
											>{currentChild.child_age || 'Not set'}</span
										>
									</div>
									<div
										class="flex justify-between py-2 border-b border-gray-200 dark:border-gray-700"
									>
										...
										<span class="text-gray-900 dark:text-white"
											>{currentChild.child_gender || 'Not set'}</span
										>
									</div>
						...
						<div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
							<div class="text-sm text-gray-500 dark:text-gray-400 mb-6">
								This profile helps personalize the AI learning experience for {currentChild.name ||
									'your child'}.
							</div>
