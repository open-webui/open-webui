<script lang="ts">
	...
	import { WEBUI_NAME, mobile, showSidebar, user, sidebarPinned } from '$lib/stores';
...
{#if loaded}
	<div
		class=" flex flex-col h-screen max-h-[100dvh] flex-1 transition-width duration-200 ease-in-out {$sidebarPinned
			? 'md:max-w-[calc(100%-var(--sidebar-width))]'
			: ' md:max-w-[calc(100%-52px)]'}  w-full max-w-full"
	>
			...
			<div class=" flex items-center gap-1">
				...
				<div class=" flex w-full">
					...
					>
						...
						<a
							class="min-w-fit p-1.5 {$page.url.pathname.includes('/admin/roles')
								? ''
								: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition"
