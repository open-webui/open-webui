<script lang="ts">
	...
	const handleKeydown = (e) => {
		// Ctrl+Shift+L to toggle login form
	...
	const oauthCallbackHandler = async () => {
		...
		console.log('[OAuth] token from cookie:', token ? 'found' : 'not found');
		if (!token) {
			return;
		}

		const sessionUser = await getSessionUser(token).catch((error) => {
			console.error('[OAuth] getSessionUser error:', error);
			toast.error(`${error}`);
			return null;
		});

		console.log('[OAuth] sessionUser:', sessionUser);
		if (!sessionUser) {
			return;
		}
...
<div class="w-full h-screen max-h-[100dvh] relative overflow-hidden" id="auth-page">
	...
	{#if loaded}
		...
		>
			<div
				class="w-full px-4 sm:px-10 min-h-screen flex flex-col justify-center items-center py-10 gap-20"
			>
				{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
					...
					<div class="flex flex-col items-center gap-5 sm:gap-10">
						...
						<div class="flex flex-col items-center gap-4">
							<div class="flex items-center gap-1 sm:gap-4">
								<div
									class="w-[43px] h-[43px] sm:w-[72px] sm:h-[72px] flex items-center justify-center scale-[1.2] sm:scale-[2] origin-center"
								>
									<HYULogo36 />
								</div>
								<span class="text-title-4 sm:text-title-1 tracking-tight"
									>HYU AI Tutoring Assistant</span
								>
							</div>
						</div>
						<!-- Welcome Messages -->
					...
					>
						...
						>
							<svg
								...
								class="w-12 h-12 sm:w-16 sm:h-16 text-gray-950 dark:text-gray-50"
								viewBox="0 0 24 24"
								fill="currentColor"
							>
								<path
						...
						<!-- Title and Caption (tap 5 times to show login form) -->
						<div class="flex flex-col items-center w-full">
							<!-- svelte-ignore a11y-no-static-element-interactions -->
							<h2
								class="text-title-3 sm:text-title-2 text-center cursor-default select-none"
						...
						<!-- Login Form (Hidden by default, Ctrl+Shift+L to show) -->
						{#if showLoginForm}
							<form
								...
								{#if $config?.features.enable_login_form || $config?.features.enable_ldap || form}
									{#if mode === 'signup'}
										<div class="flex flex-col gap-1">
											<label for="name" class="text-body-4-medium text-left"
												>{$i18n.t('Name')}</label
											>
											<input
												bind:value={name}
									...
									{#if mode === 'ldap'}
										<div class="flex flex-col gap-1">
											<label for="username" class="text-body-4-medium text-left"
									...
									{:else}
										<div class="flex flex-col gap-1">
											<label for="email" class="text-body-4-medium text-left"
												>{$i18n.t('Email')}</label
											>
											<input
												bind:value={email}
									...
									<div class="flex flex-col gap-1">
										<label for="password" class="text-body-4-medium text-left"
									...
									{#if mode === 'signup' && $config?.features?.enable_signup_password_confirmation}
										<div class="flex flex-col gap-1">
											<label for="confirm-password" class="text-body-4-medium text-left"
						...
						<div class="flex flex-col items-center gap-2 mt-8">
							<button
								class="flex items-center justify-center gap-2 px-5 bg-primary-600 sm:px-7 py-1.5 sm:py-2 rounded-full text-body-3-medium sm:text-body-2-medium text-white transition hover:opacity-90 w-full"
								type="button"
								on:click={() => {
									window.location.href = '/oauth/hanyang/login';
								}}
							>
								<svg
									width="24"
						...
						<!-- {#if Object.keys($config?.oauth?.providers ?? {}).length > 0}
							<div class="inline-flex items-center justify-center w-full">
								...
								<span class="px-3 text-body-4 text-gray-700 dark:text-gray-300"
									>{$i18n.t('or')}</span
								>
								<hr class="w-24 h-px border-0 bg-gray-200/50 dark:bg-gray-600/30" />
							</div>
...
<style>
	...
	.icon-circle {
		...
		box-shadow:
			...
			inset 2px 2px 16px rgba(206, 212, 229, 0.12);
