...
<div class="w-full text-white relative bg-auth">
	{#if loaded}
		<div class="login min-h-screen w-full flex justify-center relative">
			...
			<div
				class="login__right relative z-20 p-[16px] flex-1 flex items-center justify-center text-center"
			>
				<div class="login-box p-[32px] w-full md:w-[440px] rounded-[24px]">
					{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
			...
			<div
				class="absolute bottom-0 left-0 right-0 text-center text-xs text-white bg-[#072d5a] py-3 rounded-b-lg"
			>
				<strong>Disclaimer:</strong> GovGPT is powered by artificial intelligence and may occasionally
				produce incorrect or outdated responses. Please verify critical information from official sources
				or consult your department head for confirmation.
			</div>
		</div>
	{/if}
</div>
