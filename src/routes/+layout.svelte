			...
			if ($config) {
				...
				if (localStorage.token) {
					...
					try {
						...
						console.log('[Session] Error getting session user:', error);
						// 檢查是否為 401 錯誤
						const errorStr = String(error);
						if (errorStr.includes('Unauthorized') || errorStr.includes('Invalid token') || errorStr.includes('401')) {
					...
					if (sessionUser) {
						...
						console.log('[Session] Invalid session, redirecting to auth page');
