...
RUN set -e; \
    ...
    # rapidocr depends on the GUI opencv-python build; keep only the pinned headless one
    echo opencv-python > /tmp/uv_excludes.txt; \
    if [ "$USE_SLIM" = "true" ]; then \
    uv pip install --system -r requirements-slim.txt --excludes /tmp/uv_excludes.txt --no-cache-dir; \
    elif [ "$USE_CUDA" = "true" ]; then \
    # If you use CUDA the whisper and embedding model will be downloaded on first use
    # fix: pin torch<=2.9.1 - torch 2.10.0 aarch64 wheels cause SIGILL on ARM devices (RPi 4 Cortex-A72) #21349
    pip3 install 'torch<=2.9.1' torchvision torchaudio --index-url https://download.pytorch.org/whl/$USE_CUDA_DOCKER_VER --no-cache-dir; \
    uv pip install --system -r requirements.txt --excludes /tmp/uv_excludes.txt --no-cache-dir; \
