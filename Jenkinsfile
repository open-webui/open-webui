    ...
    stage("deploy services") {
      ...
      steps {
        sshagent(credentials: ['JENKINS_PRIVATE_SSH_KEY']) {
          sh "ssh ubuntu@${env.INFO_SERVER_IP} docker pull ${env.IMAGE}"
          sh "ssh ubuntu@${env.INFO_SERVER_IP} docker compose -f ${env.DEPLOY_ROOT_PATH}/docker-compose.yml up -d"
          sh "ssh ubuntu@${env.INFO_SERVER_IP} docker compose -f ${env.DEPLOY_ROOT_PATH}/docker-compose.yml restart nginx"
        }
      }
    }
  }
