...
def get_builtin_tools(
    ...
    ) and get_model_capability("web_search") and features.get("web_search"):
        builtin_functions.extend([search_web, fetch_url])

    # Add image generation/edit tools if enabled globally AND model has image_generation capability
    if getattr(
        request.app.state.config, "ENABLE_IMAGE_GENERATION", False
    ) and get_model_capability("image_generation") and features.get("image_generation"):
    ...
    if (
        ...
        and get_model_capability("code_interpreter") and features.get("code_interpreter")
    ):
