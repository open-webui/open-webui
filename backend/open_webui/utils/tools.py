...
async def get_tools(
    ...
    for tool_id in tool_ids:
        ...
        if tool:
            ...
            if (
                module is None
                or request.app.state.TOOL_CONTENTS.get(tool_id) != tool.content
            ):
                    ...
                    for spec in specs:
                        ...
                        if connection_headers and isinstance(connection_headers, dict):
                            ...
                            custom_headers = get_custom_headers(
                                connection_headers, user, metadata
                            )
    ...
    for keyword in ("oneOf", "anyOf", "allOf"):
        if keyword in resolved_schema and isinstance(resolved_schema[keyword], list):
            resolved_schema[keyword] = [
                resolve_schema(inner, components, resolved_schemas)
                for inner in resolved_schema[keyword]
            ]
...
async def set_tool_servers(request: Request):
    try:
        ...
        request.app.state.TOOL_SERVERS = (
            getattr(request.app.state, "TOOL_SERVERS", None) or []
        )

    try:
        if request.app.state.redis is not None:
            await request.app.state.redis.set(
                f"{REDIS_KEY_PREFIX}:tool_servers",
                json.dumps(request.app.state.TOOL_SERVERS),
            )
    except Exception as e:
...
async def get_terminal_system_prompt(
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            async with session.get(
                f"{base}/api/config", ssl=AIOHTTP_CLIENT_SESSION_SSL
            ) as resp:
                if resp.status != 200:
                    return None
...
async def set_terminal_servers(request: Request):
    ...
    if request.app.state.redis is not None:
        await request.app.state.redis.set(
            f"{REDIS_KEY_PREFIX}:terminal_servers",
            json.dumps(request.app.state.TERMINAL_SERVERS),
        )
...
async def get_terminal_tools(
    ...
    for spec in specs:
        ...
        tool_function = await make_tool_function(
            function_name, server_data, headers, cookies
        )
        callable = await get_async_tool_function_and_apply_extra_params(
            tool_function, {}
        )
