...
async def get_all_models(request, refresh: bool = False, user: UserModel = None):
    ...
    # Identity-based like list.remove, but without an O(n) scan per removal
    removed_model_obj_ids = set()
        ...
        if custom_model.base_model_id is None:
            ...
            if model:
                if custom_model.is_active:
                    ...
                    removed_model_obj_ids.add(id(model))
    ...
    if removed_model_obj_ids:
        models = [model for model in models if id(model) not in removed_model_obj_ids]
