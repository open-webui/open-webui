                            ...
                            if data:
                                ...
                                # Check for Responses API events (type field starts with "response.")
                                    ...
                                    if raw_usage:
                                        ...
                                        captured_usage.update(raw_usage)
                                        await event_emitter(
                                            {
                                                'type': 'chat:completion',
                                                'data': {
                    ...
                    try:
                        UserUsages.record_usage(
                            UsageRecordForm(
                                user_id=user.id,
                                model_id=model_id,
                                input_tokens=captured_usage.get("input_tokens", 0)
                                or captured_usage.get("prompt_tokens", 0)
                                or 0,
                                output_tokens=captured_usage.get("output_tokens", 0)
                                or captured_usage.get("completion_tokens", 0)
                                or 0,
                                reasoning_tokens=(
                                    captured_usage.get("output_tokens_details", {}).get("reasoning_tokens", 0)
                                    if captured_usage.get("output_tokens_details")
                                    else 0
                                ),
