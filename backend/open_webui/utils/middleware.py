...
async def process_chat_payload(request, form_data, user, metadata, model):
        ...
        if db_messages:
            ...
            if assistant_message_id:
                ...
                if assistant_message and (
                    assistant_message.get("content") or assistant_message.get("output")
                ):
                    db_messages.append(
                        {
                            k: v
                            for k, v in assistant_message.items()
                            if k in ("role", "content", "output", "files")
                        }
                    )
    ...
    if folder_id and user:
        ...
        if folder and folder.data:
            ...
            if "files" in folder.data:
                ...
                allowed_files = await get_accessible_folder_files(
                    folder.data["files"], user
                )
    ...
    if all_skill_ids:
        ...
        accessible_skill_ids = {
            s.id for s in await SkillsModel.get_skills_by_user_id(user.id, "read")
        }
        available_skills = []
            ...
            if file_item.get("type", "file") == "folder":
                ...
                if folder_id:
                    ...
                    if folder and folder.data and "files" in folder.data:
                        ...
                        files = [
                            *files,
                            *await get_accessible_folder_files(
                                folder.data["files"], user
                            ),
                        ]
...
async def background_tasks_handler(ctx):
    ...
    if message and "model" in message:
        if tasks and messages:
            ...
            if not metadata.get("chat_id", "").startswith(
                "local:"
            ) and not metadata.get("chat_id", "").startswith(
                "channel:"
...
async def outlet_filter_handler(ctx):
    ...
    try:
        ...
        models = request.app.state.MODELS
        try:
            outlet_data = await process_pipeline_outlet_filter(
                request, outlet_data, user, models
            )
        except Exception as e:
...
async def non_streaming_chat_response_handler(response, ctx):
    ...
    if event_emitter:
        try:
            ...
            if "selected_model_id" in response_data and not metadata[
                "chat_id"
            ].startswith("channel:"):
                await Chats.upsert_message_to_chat_by_id_and_message_id(
                    metadata["chat_id"],
            ...
            if choices and choices[0].get("message", {}).get("content"):
                ...
                if content:
                    ...
                    if (
                        request.app.state.config.ENABLE_USER_WEBHOOKS
        ...
        async def response_handler(response, events):
            ...
            try:
                ...
                async def stream_body_handler(response, form_data):
                            ...
                            if data:
                                ...
                                # Check for Responses API events (type field starts with "response.")
                                            ...
                                            for part in item.get("content", []):
                                                for annotation in part.get(
                                                    ...
                                                    if (
                                                        ...
                                                        url_citation = annotation.get(
                                                            "url_citation", annotation
                                                        )

                                                        url = url_citation.get(
                                                            "url", ""
                                                        )
                                                        title = url_citation.get(
                                                            "title", url
                                                        )
                                    ...
                                    if not choices:
                                        ...
                                        if error:
                                            log.error(
                                                "Provider returned error (streaming): %s",
                                                error,
                                            )
                                            try:
                                                await Chats.upsert_message_to_chat_by_id_and_message_id(
                                    ...
                                    if value:
                                        ...
                                        if ENABLE_CHAT_RESPONSE_BASE64_IMAGE_URL_CONVERSION:
                                            value = (
                                                await convert_markdown_base64_images(
                                                    request,
                                                    value,
                                                    {
                                        ...
                                        if ENABLE_REALTIME_CHAT_SAVE and not metadata[
                                            "chat_id"
                                        ].startswith("channel:"):
                                            # Save message in the database
                                            await Chats.upsert_message_to_chat_by_id_and_message_id(
                    ...
                    if not response_tool_calls and output:
                        ...
                        if responses_api_tool_calls:
                            tool_calls.append(
                                _split_tool_calls(responses_api_tool_calls)
                            )
                ...
                while (
                    ...
                    for result in results:
                        output_parts = [
                            {"type": "input_text", "text": result.get("content", "")}
                        ]
                    ...
                    while (
                        ...
                        try:
                            if ci_item.get("attributes", {}).get("type") == "code":
                                ...
                                if isinstance(ci_output, dict) and ci_output.get(
                                    "error"
                                ):
                                    ...
                                    if isinstance(stdout, str):
                                        ...
                                        for idx, line in enumerate(stdoutLines):
                                            if re.match(r"data:image/\w+;base64", line):
                                                image_url = (
                                                    await get_image_url_from_base64(
                                                        request,
                                                        line,
                                                        metadata,
                                                        user,
