        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, content, output):
                ...
                if last_type == "message":
                    text_content = get_last_text(output)

                    # Check if text starts with an end tag (e.g., </think>) that should
                    ...
                    for start_tag, end_tag in tags:
                        if text_content.lstrip().startswith(end_tag):
                            # Look for an unclosed reasoning block earlier in output
                            for i in range(len(output) - 2, -1, -1):
                                block = output[i]
                                if (block.get("type") == output_type_map.get(content_type, content_type)
                                    ...
                                    # Remove the end tag from the text
                                    text_content = text_content.lstrip()[len(end_tag):].lstrip()
                                    set_last_text(output, text_content)
                                    break
                            break
