    ...
    if files := body.get('metadata', {}).get('files', None):
        ...
        if len(queries) == 0:
            ...
            if not user_message_text:
                ...
                try:
                    queries = await generate_image_rag_queries(request, body, user)
                    if queries:
                        log.debug(f'Generated image RAG queries: {queries}')
                        await __event_emitter__(
                            {
                                'type': 'status',
                                'data': {
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if file_context_enabled:
        try:
            _model_rag_top_k = model.get('info', {}).get('meta', {}).get('rag_top_k') if model else None
            form_data, flags = await chat_completion_files_handler(
                request, form_data, extra_params, user, model_rag_top_k=_model_rag_top_k
            )
            sources.extend(flags.get('sources', []))
