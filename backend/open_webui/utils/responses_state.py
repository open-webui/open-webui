...
def convert_to_responses_payload(payload: dict) -> dict:
    ...
    legacy_functions = responses_payload.pop('functions', None)
    if legacy_functions and not responses_payload.get('tools'):
        responses_payload['tools'] = [{'type': 'function', **function} for function in legacy_functions]

    legacy_function_call = responses_payload.pop('function_call', None)
    if legacy_function_call is not None and 'tool_choice' not in responses_payload:
        if isinstance(legacy_function_call, dict):
            ...
            responses_payload['tool_choice'] = legacy_function_call
    ...
    if 'tool_choice' in responses_payload:
        responses_payload['tool_choice'] = _convert_responses_tool_choice(responses_payload['tool_choice'])

    stream_options = responses_payload.get('stream_options')
    if isinstance(stream_options, dict) and 'include_obfuscation' in stream_options:
        responses_payload['stream_options'] = {'include_obfuscation': stream_options['include_obfuscation']}
    else:
        responses_payload.pop('stream_options', None)
