...
def get_connection_oauth_authorize_params(connection: dict) -> dict[str, str] | None:
    ...
    if not isinstance(value, dict):
        log.warning('oauth_authorize_params is not a dict, ignoring')
        return {}
    ...
    for key, item in value.items():
        ...
        if isinstance(item, bool):
            # JSON booleans are lowercase on the wire; str(True) would send 'True'.
            params[str(key)] = 'true' if item else 'false'
        elif isinstance(item, (str, int, float)):
            params[str(key)] = str(item)
        else:
            log.warning('oauth_authorize_params value for %s is not a scalar, ignoring', key)
...
def default_oauth_authorize_params(client_info: OAuthClientInformationFull) -> dict[str, str]:
    ...
    """
    client_id = f'{client_info.client_id or ""}'
    if not client_id.endswith(GOOGLE_CLIENT_ID_SUFFIX):
        return {}

    # client_id alone is admin-supplied and not a reliable provider signal.
    # Require the discovered issuer / authorization endpoint to agree it is
    # Google before sending Google-specific parameters, so a non-Google IdP
    # that happens to use a similar client_id does not receive them.
    hosts: list[str] = []
    if client_info.issuer:
        try:
            hosts.append(urllib.parse.urlparse(client_info.issuer).netloc.lower())
        except Exception:
            pass
    md = client_info.server_metadata
    if md is not None:
        for attr in ('issuer', 'authorization_endpoint'):
            val = getattr(md, attr, None)
            if val:
                try:
                    hosts.append(urllib.parse.urlparse(str(val)).netloc.lower())
                except Exception:
                    pass
    if hosts:
        if not any(h == 'accounts.google.com' for h in hosts):
            return {}
