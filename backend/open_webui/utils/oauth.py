...
async def get_protected_resource_metadata(server_url: str) -> ProtectedResourceMetadata:
            ...
            async with session.post(
                ...
                if response.status == 401:
                    ...
                    if match:
                        ...
                        log.debug(
                            f"No resource_metadata in header, trying well-known URIs: {resource_metadata_urls}"
                        )
                        ...
                        try:
                            async with session.get(
                                ...
                                if resource_response.status == 200:
                                    ...
                                    if resource:
                                        log.debug(
                                            f"Discovered resource indicator: {resource}"
                                        )
                            ...
                            log.debug(
                                f"Failed to fetch resource metadata from {resource_metadata_url}: {e}"
                            )
                            continue
    except Exception as e:
    ...
    return ProtectedResourceMetadata(
        resource=resource, authorization_servers=authorization_servers
    )
...
def _build_well_known_urls(server_url: str) -> list[str]:
    ...
    if parsed.path and parsed.path != "/":
        ...
        urls.extend(
            [
                urllib.parse.urljoin(
                    base_url, f"/.well-known/oauth-authorization-server{path}"
                ),
                urllib.parse.urljoin(
    ...
    def add_client(self, client_id, oauth_client_info: OAuthClientInformationFull):
        kwargs = {
            ...
            "client_kwargs": {
                ...
                **(
                    {"timeout": int(OAUTH_CLIENT_TIMEOUT.value)}
                    if OAUTH_CLIENT_TIMEOUT.value
                    else {}
                ),
                **(
        ...
        if (
            ...
            and "S256"
            not in oauth_client_info.server_metadata.code_challenge_methods_supported
        ):
    ...
    async def get_oauth_token(
        ...
        try:
            ...
            if (
                ...
                or datetime.now() + timedelta(minutes=5)
...
class OAuthManager:
    ...
    async def get_user_role(self, user, user_data):
        ...
        if not user and user_count == 0:
            ...
            log.debug(
                "First user bootstrap: using default role (admin promotion deferred to post-insert)"
            )
    ...
    async def handle_login(self, request, provider):
        ...
        if client is None:
            raise HTTPException(404)
        redirect_uri = (client.server_metadata or {}).get(
            "redirect_uri"
            ...
            if not user:
                ...
                if auth_manager_config.OAUTH_MERGE_ACCOUNTS_BY_EMAIL:
                    ...
                    if user:
                        ...
                        await Users.update_user_oauth_by_id(
                            user.id, provider, sub, db=db
                        )
                    ...
                    if email_claim:
                        ...
                        if new_email and new_email.lower() != user.email.lower():
                            ...
                            if existing_user:
                                ...
                                await Auths.update_email_by_id(
    ...
    async def handle_backchannel_logout(self, request, db=None):
        ...
        try:
            ...
            return JSONResponse(
                ...
                content={
                    "error": "invalid_request",
                    "error_description": "Malformed logout_token",
                },
            )
        ...
        for provider_name in OAUTH_PROVIDERS:
            ...
            try:
                ...
                log.debug(
                    f"Back-channel logout: error checking provider {provider_name}: {e}"
                )
                continue

        if not matched_provider or not matched_client_id or not matched_jwks_uri:
            log.warning(
                f"Back-channel logout: no configured provider matches issuer {token_issuer}"
            )
            return JSONResponse(
                status_code=400,
                content={
        ...
        try:
            ...
            return JSONResponse(
                ...
                content={
                    "error": "invalid_request",
                    "error_description": f"Invalid logout_token: {e}",
                },
            )
        except Exception as e:
            ...
            return JSONResponse(
                ...
                content={
                    "error": "invalid_request",
                    "error_description": "Failed to validate logout_token",
                },
            )
        ...
        if "http://schemas.openid.net/event/backchannel-logout" not in events:
            log.warning(
                "Back-channel logout: missing required backchannel-logout event claim"
            )
            return JSONResponse(
                status_code=400,
                content={
        ...
        if "nonce" in claims:
            log.warning(
                "Back-channel logout: logout_token contains nonce (rejected per spec)"
            )
            return JSONResponse(
                status_code=400,
                content={
        ...
        if not sub and not sid:
            log.warning(
                "Back-channel logout: logout_token contains neither sub nor sid"
            )
            return JSONResponse(
                status_code=400,
                content={
        ...
        if not users_to_logout and sid:
            log.debug(
                f"Back-channel logout: no user found by sub, sid-based lookup not yet supported (sid={sid})"
            )

        if not users_to_logout:
            log.debug(
                f"Back-channel logout: no matching user for provider={matched_provider}, sub={sub}, sid={sid}"
            )
