                ...
                if len(versions) > 1:
                                ...
                                while queue:
                                    ...
                                    if current.down_revision:
                                        if isinstance(current.down_revision, tuple):
                                            ...
                                            r = script.get_revision(current.down_revision)
                                            if r:
                                                queue.append(r)
                            except Exception:
                                pass
                    ...
                    if to_remove:
                        ...
                        for old_ver in to_remove:
                            conn.execute(
                                sa_text("DELETE FROM alembic_version WHERE version_num = :ver"),
                                {"ver": old_ver},
                            )
                        conn.commit()
...
GMAIL_PERIODIC_SYNC_INTERVAL_HOURS = PersistentConfig(
    ...
    int(os.environ.get("GMAIL_PERIODIC_SYNC_INTERVAL_HOURS", "0")),  # 0 = use minutes instead
)
...
GCS_UPLOAD_TIMEOUT_SECONDS = int(os.environ.get("GCS_UPLOAD_TIMEOUT_SECONDS", "300"))  # 5 minutes
GCS_DOWNLOAD_TIMEOUT_SECONDS = int(os.environ.get("GCS_DOWNLOAD_TIMEOUT_SECONDS", "300"))  # 5 minutes
...
CHUNK_SIZE = PersistentConfig('CHUNK_SIZE', 'rag.chunk_size', int(os.environ.get('CHUNK_SIZE', '1500')))
