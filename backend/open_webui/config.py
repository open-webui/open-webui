...
DEEPSEEK_RAG_TEMPLATE = """
# 以下内容是用户上传的文件和搜索结果的结合:
{{CONTEXT}}
在我给你的搜索结果和文件中，每个结果都是[file X begin]...[file X end]格式的，X代表每篇文章的数字索引。请在适当的情况下在句子末尾引用上下文。请按照引用编号[X]的格式在答案中对应部分引用上下文。如果一句话源自多个上下文，请列出所有相关的引用编号，例如[3]、[5]，切记不要将引用集中在最后返回引用编号，而是在答案对应部分列出。
在回答时，请注意以下几点：
- 今天是{{CURRENT_DATE}}。
...
RAG_DEEPSEEK_TEMPLATE = PersistentConfig(
    "DEEPSEEK_RAG_TEMPLATE",
    "rag.deepseek_template",
    os.environ.get("DEEPSEEK_RAG_TEMPLATE", DEEPSEEK_RAG_TEMPLATE),
)
