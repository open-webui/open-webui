...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "is_protected", sa.Boolean(), default=False
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.BigInteger(), nullable=False),
        sa.UniqueConstraint(
            "chat_id", "message_id", "tag_id", name="uq_chat_message_tag"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "custom_tagging_prompt", sa.Text(), nullable=True
        ),  # Custom prompt for AI tagging
        sa.Column(
            "custom_system_instruction", sa.Text(), nullable=True
    ...
    op.create_index(
        "idx_message_tag_chat_message", "message_tag", ["chat_id", "message_id"]
    )
    ...
    op.create_index(
        "idx_message_tag_def_usage", "message_tag_definition", ["usage_count"]
    )
