...
def upgrade() -> None:
    ...
    with op.batch_alter_table("tagging_daemon_config", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("enable_rag_chapter_detection", sa.Boolean(), nullable=True)
        )

    # Set default value for existing rows (False for backward compatibility)
    op.execute(
        "UPDATE tagging_daemon_config SET enable_rag_chapter_detection = 0 WHERE enable_rag_chapter_detection IS NULL"
    )

    # Make column non-nullable after setting defaults
    with op.batch_alter_table("tagging_daemon_config", schema=None) as batch_op:
        batch_op.alter_column(
            "enable_rag_chapter_detection", nullable=False, server_default=sa.false()
        )
