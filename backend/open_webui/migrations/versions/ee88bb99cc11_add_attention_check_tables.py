...
def upgrade() -> None:
    ...
    if "attention_check_question" not in existing_tables:
        ...
        op.create_index(
            "idx_acq_created_at", "attention_check_question", ["created_at"]
        )
    else:
        # Table exists, check if index exists
        existing_indexes = [
            idx["name"] for idx in inspector.get_indexes("attention_check_question")
        ]
    ...
    if "attention_check_response" not in existing_tables:
        ...
        op.create_index(
            "idx_acr_created_at", "attention_check_response", ["created_at"]
        )
    else:
        # Table exists, check if indexes exist
        existing_indexes = [
            idx["name"] for idx in inspector.get_indexes("attention_check_response")
        ]
        indexes_to_create = [
