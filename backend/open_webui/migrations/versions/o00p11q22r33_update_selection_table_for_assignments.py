...
def upgrade() -> None:
    ...
    if "selection" in inspector.get_table_names():
        ...
        if "assignment_id" not in columns:
            ...
            op.create_index(
                "idx_selection_assignment_id", "selection", ["assignment_id"]
            )
        ...
        if "start_offset" not in columns:
            op.add_column(
                "selection", sa.Column("start_offset", sa.Integer(), nullable=True)
            )

        # Add end_offset if it doesn't exist
        if "end_offset" not in columns:
            op.add_column(
                "selection", sa.Column("end_offset", sa.Integer(), nullable=True)
            )
