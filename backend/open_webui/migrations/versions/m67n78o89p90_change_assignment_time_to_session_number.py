...
def upgrade() -> None:
    ...
    if "assignment_session_activity" in existing_tables:
        ...
        if "attempt_number" in existing_columns and "session_number" not in existing_columns:
        ...
        if "idx_assignment_activity_user_session" not in existing_indexes and "session_number" in existing_columns:
            op.create_index(
...
def downgrade() -> None:
    ...
    if "assignment_session_activity" in existing_tables:
        ...
        if "session_number" in existing_columns and "attempt_number" not in existing_columns:
            with op.batch_alter_table("assignment_session_activity") as batch_op:
                batch_op.alter_column("session_number", new_column_name="attempt_number")
        
        # Recreate old index if it doesn't exist
        if "idx_assignment_activity_user_attempt" not in existing_indexes and "attempt_number" in existing_columns:
            op.create_index(
