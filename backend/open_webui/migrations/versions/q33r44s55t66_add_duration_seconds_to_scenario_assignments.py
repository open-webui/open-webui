...
def upgrade() -> None:
    ...
    if "scenario_assignments" in existing_tables:
        existing_columns = [col["name"] for col in inspector.get_columns("scenario_assignments")]
        if "duration_seconds" not in existing_columns:
            op.add_column('scenario_assignments', sa.Column('duration_seconds', sa.Integer(), nullable=True))
