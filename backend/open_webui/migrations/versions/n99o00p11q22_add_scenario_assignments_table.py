...
def upgrade() -> None:
    ...
    if "scenario_assignments" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "alpha", sa.Float(), nullable=True
            ),  # Weighted sampling alpha parameter
            sa.Column("eligible_pool_size", sa.Integer(), nullable=True),
            sa.Column(
                "n_assigned_before", sa.Integer(), nullable=True
            ),  # n_assigned at time of assignment
            sa.Column("weight", sa.Float(), nullable=True),  # Calculated weight
            sa.Column(
                "sampling_prob", sa.Float(), nullable=True
            ),  # Realized sampling probability
            sa.Column(
                "assignment_position", sa.Integer(), nullable=True
            ),  # Position in session (0-indexed)
            # Outcome fields
            sa.Column("issue_any", sa.Integer(), nullable=True),  # 0, 1, or NULL
            sa.Column(
                "skip_stage", sa.String(), nullable=True
            ),  # Stage where skip occurred
            sa.Column(
                "skip_reason", sa.String(), nullable=True
            ),  # Reason code for skip
            sa.Column(
                "skip_reason_text", sa.Text(), nullable=True
            ),  # Optional text explanation
        )

        # Create indexes for efficient querying
        op.create_index(
            "idx_assignments_participant_id", "scenario_assignments", ["participant_id"]
        )
        op.create_index(
            "idx_assignments_scenario_id", "scenario_assignments", ["scenario_id"]
        )
        op.create_index("idx_assignments_status", "scenario_assignments", ["status"])
        op.create_index(
            "idx_assignments_assigned_at", "scenario_assignments", ["assigned_at"]
        )
        op.create_index(
            "idx_assignments_participant_scenario",
            "scenario_assignments",
...
def downgrade() -> None:
    op.drop_index(
        "idx_assignments_participant_scenario", table_name="scenario_assignments"
    )
