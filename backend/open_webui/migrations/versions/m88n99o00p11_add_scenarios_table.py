...
def upgrade() -> None:
    ...
    if "scenarios" not in existing_tables:
        op.create_table(
            ...
            # Metadata fields
            sa.Column('set_name', sa.String(), nullable=True),  # 'pilot', 'scaled', etc.
            sa.Column('trait', sa.String(), nullable=True),  # 'Agreeableness', 'Conscientiousness', etc.
            sa.Column('polarity', sa.String(), nullable=True),  # 'positive', 'negative', 'neutral'
            sa.Column('prompt_style', sa.String(), nullable=True),  # 'Journalistic', 'Should I', etc.
            sa.Column('domain', sa.String(), nullable=True),  # 'Internet Interaction', 'Self', etc.
            sa.Column('is_validated', sa.Boolean(), nullable=False, server_default='0'),
            
            # Source tracking fields
            sa.Column('source', sa.String(), nullable=True),  # 'json_file', 'api_generated', 'manual'
            sa.Column('model_name', sa.String(), nullable=True),  # Model that produced the response
            sa.Column('is_active', sa.Boolean(), nullable=False, server_default='1'),  # Whether to show to users
