...
def upgrade() -> None:
    ...
    if "attention_check_scenarios" not in existing_tables:
        ...
        op.create_index('idx_ac_scenarios_is_active', 'attention_check_scenarios', ['is_active'])
        op.create_index('idx_ac_scenarios_set_name', 'attention_check_scenarios', ['set_name'])
        op.create_index('idx_ac_scenarios_trait_theme', 'attention_check_scenarios', ['trait_theme'])
...
def downgrade() -> None:
    ...
    if "attention_check_scenarios" in existing_tables:
        # Drop indexes first
        existing_indexes = [idx["name"] for idx in inspector.get_indexes("attention_check_scenarios")]
        if "idx_ac_scenarios_trait_theme" in existing_indexes:
            op.drop_index('idx_ac_scenarios_trait_theme', table_name='attention_check_scenarios')
        if "idx_ac_scenarios_set_name" in existing_indexes:
            op.drop_index('idx_ac_scenarios_set_name', table_name='attention_check_scenarios')
        if "idx_ac_scenarios_is_active" in existing_indexes:
            op.drop_index('idx_ac_scenarios_is_active', table_name='attention_check_scenarios')
