...
def upgrade():
    ...
    if "user" in existing_tables:
            ...
            if col_name not in user_columns:
                if col_name == "session_number":
                    op.add_column(
                        "user",
                        sa.Column(
                            col_name, col_type, nullable=False, server_default="1"
                        ),
                    )
                else:
        ...
        if not any(
            constraint["name"] == "uq_user_prolific_pid"
    ...
    if "moderation_session" in existing_tables:
        ...
        existing_indexes = [
            idx["name"] for idx in inspector.get_indexes("moderation_session")
        ]
...
def downgrade():
    ...
    if "moderation_session" in existing_tables:
        ...
        if "idx_mod_session_user_session" in existing_indexes:
            op.drop_index(
                "idx_mod_session_user_session", table_name="moderation_session"
            )
        if "idx_mod_session_composite" in existing_indexes:
            op.drop_index("idx_mod_session_composite", table_name="moderation_session")
        moderation_session_columns = [
