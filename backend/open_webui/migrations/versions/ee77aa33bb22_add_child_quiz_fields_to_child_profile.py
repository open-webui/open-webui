        ...
        for col_name, col_type, _ in columns_to_add:
            if col_name not in child_profile_columns:
                if col_name == "child_ai_use_contexts":
                    ...
                    try:
                        op.add_column('child_profile', sa.Column(col_name, sa.JSON(), nullable=True))
                    except Exception:
                        op.add_column('child_profile', sa.Column(col_name, sa.Text(), nullable=True))
                else:
                    op.add_column('child_profile', sa.Column(col_name, col_type, nullable=True))
