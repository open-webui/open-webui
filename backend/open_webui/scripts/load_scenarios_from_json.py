...
def load_scenarios_from_json(
    json_file_path: str, set_name: str = "pilot", source: str = "json_file"
):
    """
    ...
    for idx, scenario_data in enumerate(scenarios_data, 1):
        try:
            ...
            if not prompt_text or not response_text:
                print(
                    f"Warning: Scenario {idx} missing prompt or response, skipping..."
                )
                error_count += 1
                continue
            ...
            form = ScenarioForm(
                ...
                model_name=scenario_data.get(
                    "model_name"
                ),  # May not be in JSON, will be None
                is_active=True,  # Active by default
            )

            # Upsert scenario (will create or update)
            existing = (
                Scenarios.get_by_id(form.scenario_id) if form.scenario_id else None
            )
            ...
            if existing:
                updated_count += 1
                print(
                    f"  [{idx}/{len(scenarios_data)}] Updated scenario: {result.scenario_id}"
                )
            else:
                loaded_count += 1
                print(
                    f"  [{idx}/{len(scenarios_data)}] Loaded scenario: {result.scenario_id}"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Load scenarios from JSON file into database"
    )
    parser.add_argument(
        "--json-file",
        type=str,
