...
class RedisLock:
    def __init__(self, redis_url, lock_name, timeout_secs, cluster_mode, redis_sentinels=[]):
        self.lock_name = lock_name
        self.lock_id = str(uuid.uuid4())
        self.timeout_secs = timeout_secs
        self.lock_obtained = False
        self.redis = get_redis_connection(
            redis_url, redis_sentinels, cluster_mode=cluster_mode, decode_responses=True
        )
...
class RedisDict:
    def __init__(self, name, redis_url, cluster_mode, redis_sentinels=[]):
        self.name = name
        self.redis = get_redis_connection(
            redis_url, redis_sentinels, cluster_mode=cluster_mode, decode_responses=True
        )
