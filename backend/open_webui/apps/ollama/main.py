...
async def update_ollama_api_url(form_data: UrlUpdateForm, user=Depends(get_admin_user)):
    ...
    log.info(
        f"app.state.config.OLLAMA_BASE_URLS: {app.state.config.OLLAMA_BASE_URLS}")
    return {"OLLAMA_BASE_URLS": app.state.config.OLLAMA_BASE_URLS}
...
async def push_model(
    ...
    return await post_streaming_url(
        f"{url}/api/push", form_data.model_dump_json(
...
async def create_model(
    ...
    return await post_streaming_url(
        f"{url}/api/create", form_data.model_dump_json(
            exclude_none=True).encode()
    )
...
async def generate_completion(
    ...
    return await post_streaming_url(
        f"{url}/api/generate", form_data.model_dump_json(
            exclude_none=True).encode()
    )
