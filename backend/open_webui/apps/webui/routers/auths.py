...
async def ldap_auth(request: Request, response: Response, form_data: LdapForm):
    ...
    USE_SSL = request.app.state.config.USE_SSL
    CA_CERT_FILE = request.app.state.config.CA_CERT_FILE
    ...
    try:
        tls = Tls(validate=CERT_REQUIRED, version=PROTOCOL_TLS, ca_certs_file=CA_CERT_FILE, ciphers=LDAP_CIPHERS)
    except Exception as e:
        log.error(f"An error occurred on TLS: {str(e)}")
        raise HTTPException(400, detail=str(e))
    
    try:
        server = Server(host=LDAP_SERVER_HOST, port=LDAP_SERVER_PORT, get_info=ALL, use_ssl=USE_SSL, tls=tls)
        connection_app = Connection(server, LDAP_APP_DN, LDAP_APP_PASSWORD, auto_bind='NONE', authentication='SIMPLE')
        if not connection_app.bind():
            raise HTTPException(400, detail="Application account bind failed")

        search_success = connection_app.search(search_base=LDAP_SEARCH_BASE, search_filter=f'(&({LDAP_ATTRIBUTE_FOR_USERNAME}={form_data.user}){LDAP_SEARCH_FILTERS})', attributes=[f'{LDAP_ATTRIBUTE_FOR_USERNAME}', 'mail', 'cn'])
        if not search_success:
            raise HTTPException(400, detail="User not found in the LDAP server")
        
        entry = connection_app.entries[0]
        username = str(entry[f'{LDAP_ATTRIBUTE_FOR_USERNAME}'])
        mail = str(entry['mail'])
        cn = str(entry['cn'])
        user_dn = entry.entry_dn
        if username == form_data.user:
...
async def get_ldap_server(
    ...
    return {
        ...
        "use_tls": request.app.state.config.USE_SSL,
        "certificate_path": request.app.state.config.CA_CERT_FILE,
...
async def update_ldap_server(
    ...
    request.app.state.config.USE_SSL = form_data.use_tls
    request.app.state.config.CA_CERT_FILE = form_data.certificate_path
