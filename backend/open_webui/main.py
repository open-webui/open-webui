...
from fastapi import Depends, FastAPI, HTTPException, Request, applications, status
from fastapi.middleware.cors import CORSMiddleware
from fastapi.openapi.docs import get_swagger_ui_html
from fastapi.responses import FileResponse, JSONResponse
from fastapi.staticfiles import StaticFiles
from open_webui.config import (  # Audio; Image; Admin; Model list; Code Execution; Direct Connections; WebUI (LDAP); LDAP Group Management; WebUI (OAuth); Ollama; OpenAI; Retrieval (Web Search); Misc; Tasks; Terminal Server; Thread pool size for FastAPI/AnyIO; Tool Server Configs; WebUI
    ADMIN_EMAIL,
...
from open_webui.env import (  # OAuth Back-Channel Logout; SCIM; Admin Account Runtime Creation
...
from open_webui.tasks import (
    cleanup_task,  # Import from tasks.py
    create_task,
    has_active_tasks,
...
async def unload_model(
    ...
    if model_id in openai_models:
        ...
        key = (
            ...
            if idx < len(request.app.state.config.OPENAI_API_KEYS)
            else ""
        )
...
async def chat_completion(
    ...
    try:
        ...
        is_new_chat = (
            "parent_id" in form_data
            and form_data["parent_id"] is None
            and not form_data.get("chat_id")
        )
        ...
        user_message = form_data.pop("user_message", None) or form_data.pop(
            "parent_message", None
        )
        metadata = {
            "user_id": user.id,
        ...
        if metadata.get("chat_id") and user:
            ...
            if not chat_id.startswith("local:") and not chat_id.startswith(
                ...
                if is_new_chat:
                    ...
                    all_assistant_ids = [
                        assistant_id
                        for assistant_id in message_ids.values()
                        if assistant_id
                    ]
                    ...
                    if (
                        not await Chats.is_chat_owner(chat_id, user.id)
                        and user.role != "admin"
                    ):
                        raise HTTPException(
                    ...
                    if user_message and user_message.get("id"):
                        ...
                        if grandparent_id:
                            grandparent = await Chats.get_message_by_id_and_message_id(
    ...
    async def process_chat(request, form_data, user, metadata, model, tasks=None):
        try:
            ...
            if metadata.get("chat_id") and metadata.get("message_id"):
                ...
                try:
                    if not metadata["chat_id"].startswith("local:") and not metadata[
                        "chat_id"
                    ].startswith("channel:"):
                        await Chats.upsert_message_to_chat_by_id_and_message_id(
                            metadata["chat_id"],
