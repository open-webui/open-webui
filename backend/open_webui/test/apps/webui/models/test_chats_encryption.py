...
def test_chat_model_encrypt_decrypt_flow(monkeypatch):
    ...
    monkeypatch.setattr(chats_model, 'get_db_context', lambda db=None: _db_context(session))
    ...
    created = Chats.insert_new_chat('u1', chats_model.ChatForm(chat=chat_body, folder_id=None))
    ...
    updated = Chats.update_chat_by_id(created.id, {**created.chat, 'title': 'Updated'}, db=session)
    assert updated.chat['title'] == 'Updated'

    upserted = Chats.upsert_message_to_chat_by_id_and_message_id(created.id, 'm2', {'content': 'new message'})
