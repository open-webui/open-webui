...
class TestGroundingStep:
    ...
    def test_grounding_step_basic_functionality(self):
        ...
        assert (
            len(result.validated_documents) == 2
        )  # Should filter out weather document
    ...
    def test_grounding_step_with_openai_style_embeddings(self):
        ...
        assert all(
            "artificial intelligence" in doc["document"].lower()
    ...
    def test_grounding_step_various_thresholds(self):
        ...
        assert (
            len(result_medium.validated_documents) == 2
        )  # Two highest similarity docs
