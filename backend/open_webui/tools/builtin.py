    ...
    try:
        ...
        if engine == "pyodide":
            ...
            if isinstance(output, dict):
                ...
                if (
                    output.get("error")
                    and not output.get("stdout")
                    and not output.get("result")
                ):
                    stderr = output["error"]
                    stdout = ""
                    result = ""
...
async def search_notes(
    ...
    try:
        ...
        user_group_ids = [
            group.id for group in await Groups.get_groups_by_member_id(user_id)
        ]
...
async def update_automation(
    ...
    try:
        ...
        new_model_id = (
            model_id if model_id is not None else automation.data.get("model_id", "")
        )
        ...
        if start or end:
            ...
            try:
                end_ns = (
                    _dt_to_ns(end, tz)
                    if end
                    else int(time.time() * 1_000) * 1_000_000
                    + 365 * 86400 * 1_000_000_000_000
                )
            except (ValueError, TypeError) as e:
...
async def create_calendar_event(
    ...
    try:
        ...
        if cal.user_id != user_id and __user__.get("role") != "admin":
            ...
            user_group_ids = [
                g.id for g in await Groups.get_groups_by_member_id(user_id)
            ]
            if not await AccessGrants.has_access(
                user_id=user_id,
