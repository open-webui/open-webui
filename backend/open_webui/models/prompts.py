...
class PromptsTable:
    ...
    async def _to_prompt_model(
        ...
        prompt_data["access_grants"] = (
            ...
            else await self._get_access_grants(prompt_data["id"], db=db)
        )
    ...
    async def insert_new_prompt(
        ...
        try:
            async with get_async_db_context(db) as db:
                ...
                if result:
                    current_access_grants = await self._get_access_grants(
                        prompt_id, db=db
                    )
                    snapshot = {
                        "name": form_data.name,
    ...
    async def get_prompt_by_command(
        self, command: str, db: AsyncSession | None = None
    ) -> PromptModel | None:
        try:
            async with get_async_db_context(db) as db:
    ...
    async def update_prompt_by_command(
        ...
        try:
            async with get_async_db_context(db) as db:
                ...
                latest_history = await PromptHistories.get_latest_history_entry(
                    prompt.id, db=db
                )
    ...
    async def update_prompt_version(
        ...
        try:
            async with get_async_db_context(db) as db:
                ...
                history_entry = await PromptHistories.get_history_entry_by_id(
                    version_id, db=db
                )
    ...
    async def get_tags(self, db: AsyncSession | None = None) -> list[str]:
        try:
            async with get_async_db_context(db) as db:
                result = await db.execute(
                    select(Prompt.tags).filter(Prompt.is_active == True)
                )
                tags = set()
