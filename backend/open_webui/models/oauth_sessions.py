...
class OAuthSessionTable:
    ...
    async def get_session_by_id(
        self, session_id: str, db: AsyncSession | None = None
    ) -> OAuthSessionModel | None:
        ...
        try:
            async with get_async_db_context(db) as db:
                result = await db.execute(
                    select(OAuthSession).filter_by(id=session_id, user_id=user_id)
                )
                session = result.scalars().first()
                if session:
                    return OAuthSessionModel(
    ...
    async def get_sessions_by_user_id(
        ...
        try:
            async with get_async_db_context(db) as db:
                result = await db.execute(
                    select(OAuthSession).filter_by(user_id=user_id)
                )
    ...
    async def delete_sessions_by_user_id_and_provider(
        ...
        try:
            ...
            log.error(
                f"Error deleting OAuth sessions for user {user_id} and provider {provider}: {e}"
            )
            return False

    async def delete_sessions_by_provider(
        self, provider: str, db: AsyncSession | None = None
    ) -> bool:
