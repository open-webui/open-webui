...
class TagTable:
    ...
    async def get_tags_by_ids_and_user_id(
        ...
        async with get_async_db_context(db) as db:
            result = await db.execute(
                select(Tag).filter(Tag.id.in_(ids), Tag.user_id == user_id)
            )
            return [TagModel.model_validate(tag) for tag in result.scalars().all()]
    ...
    async def delete_tags_by_ids_and_user_id(
        ...
        try:
            async with get_async_db_context(db) as db:
                await db.execute(
                    delete(Tag).filter(Tag.id.in_(ids), Tag.user_id == user_id)
                )
                await db.commit()
                return True
        except Exception as e:
    ...
    async def ensure_tags_exist(
        self, names: list[str], user_id: str, db: AsyncSession | None = None
    ) -> None:
        ...
        async with get_async_db_context(db) as db:
            result = await db.execute(
                select(Tag.id).filter(Tag.id.in_(ids), Tag.user_id == user_id)
            )
