...
class Selection(Base):
    ...
    meta = Column(
        JSON, nullable=True
    )  # Additional metadata (model used, timestamp, etc.)

    # Assignment tracking fields (added for scenario assignment tracking)
    assignment_id = Column(
        String, ForeignKey("scenario_assignments.assignment_id"), nullable=True
    )
    start_offset = Column(
...
class SelectionTable:
    ...
    def get_selections_by_user(
        ...
        with get_db() as db:
            ...
            return [
                SelectionModel.model_validate(selection) for selection in selections
            ]
    ...
    def get_selections_by_chat(self, chat_id: str) -> list[SelectionModel]:
        ...
        with get_db() as db:
            selections = (
                db.query(Selection)
                .filter(Selection.chat_id == chat_id)
    ...
    def get_selections_by_assignment(self, assignment_id: str) -> list[SelectionModel]:
        ...
        with get_db() as db:
            selections = (
                db.query(Selection)
                .filter(Selection.assignment_id == assignment_id)
                .order_by(Selection.created_at.asc())
                .all()
            )
    ...
    def delete_selection_by_text(
        self, user_id: str, chat_id: str, selected_text: str, role: str | None = None
    ) -> int:
    ...
    def get_selection_stats(self) -> dict:
        ...
        with get_db() as db:
            ...
            assistant_selections = (
                db.query(Selection).filter(Selection.role == "assistant").count()
            )
            user_role_selections = (
                db.query(Selection).filter(Selection.role == "user").count()
            )
