...
class ModelsTable:
    async def _get_access_grants(
        self, model_id: str, db: AsyncSession | None = None
    ...
    async def _to_model_model(
        ...
        model_data["access_grants"] = (
            ...
            else await self._get_access_grants(model_data["id"], db=db)
        )
    ...
    async def update_model_by_id(
        self, id: str, model: ModelForm, db: AsyncSession | None = None
    ) -> ModelModel | None:
        try:
            async with get_async_db_context(db) as db:
    ...
    async def update_model_updated_at_by_id(
        self, id: str, db: AsyncSession | None = None
    ) -> ModelModel | None:
        try:
            async with get_async_db_context(db) as db:
