...
class FolderTable:
        ...
        try:
            async with get_async_db_context(db) as db:
                result = await db.execute(
                    select(Folder).filter_by(id=id, user_id=user_id)
                )
    ...
    async def get_folders_by_user_id(
        ...
        async with get_async_db_context(db) as db:
            ...
            return [
                FolderModel.model_validate(folder) for folder in result.scalars().all()
            ]
    ...
    async def get_folders_by_parent_id_and_user_id(
        ...
        async with get_async_db_context(db) as db:
            result = await db.execute(
                select(Folder).filter_by(parent_id=parent_id, user_id=user_id)
            )
            return [
                FolderModel.model_validate(folder) for folder in result.scalars().all()
            ]
                ...
                async def delete_children(folder):
                    ...
                    for folder_child in folder_children:
                        ...
                        child_result = await db.execute(
                            select(Folder).filter_by(id=folder_child.id)
                        )
