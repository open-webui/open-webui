...
class PromptHistoryTable:
    ...
    async def get_history_entry_by_id(
        ...
        async with get_async_db_context(db) as db:
            result = await db.execute(
                select(PromptHistory).filter(PromptHistory.id == history_id)
            )
    ...
    async def compute_diff(
        ...
        async with get_async_db_context(db) as db:
            result_from = await db.execute(
                select(PromptHistory).filter(PromptHistory.id == from_id)
            )
            from_entry = result_from.scalars().first()
            result_to = await db.execute(
                select(PromptHistory).filter(PromptHistory.id == to_id)
            )
    ...
    async def delete_history_by_prompt_id(
        ...
        async with get_async_db_context(db) as db:
            await db.execute(
                delete(PromptHistory).filter(PromptHistory.prompt_id == prompt_id)
            )
            await db.commit()
            return True
    ...
    async def delete_history_entry(
        ...
        async with get_async_db_context(db) as db:
            ...
            children_result = await db.execute(
                select(PromptHistory).filter_by(parent_id=history_id)
            )
