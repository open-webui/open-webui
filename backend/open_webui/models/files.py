...
class FilesTable:
    ...
    async def get_files_by_ids(self, ids: list[str]) -> list[FileModel]:
        async with get_db() as db:
            return [
                ...
                for file in await db.query(File)
    ...
    async def get_files_by_user_id(self, user_id: str) -> list[FileModel]:
        async with get_db() as db:
            return [
                FileModel.model_validate(file)
                for file in await db.query(File).filter_by(user_id=user_id).all()
            ]

    async def update_file_hash_by_id(self, id: str, hash: str) -> Optional[FileModel]:
        async with get_db() as db:
            try:
                file = await db.query(File).filter_by(id=id).first()
                file.hash = hash
                await db.commit()
    ...
    async def delete_file_by_id(self, id: str) -> bool:
        async with get_db() as db:
            try:
                await db.query(File).filter_by(id=id).delete()
                await db.commit()
