...
class User(Base):
    ...
    )  # For child accounts, links to parent user
...
class UserModel(BaseModel):
    ...
    # Parent-child account linking
    parent_id: Optional[str] = None  # For child accounts, links to parent user
...
class UsersTable:
    ...
    def get_user_by_prolific_pid(
        self, prolific_pid: str, db: Optional[Session] = None
    ) -> Optional[UserModel]:
        try:
            with get_db_context(db) as db:
