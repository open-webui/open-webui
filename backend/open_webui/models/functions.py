...
class FunctionsTable:
    ...
    async def get_functions_by_type(
        ...
        async with get_async_db_context(db) as db:
            if active_only:
                result = await db.execute(
                    select(Function).filter_by(type=type, is_active=True)
                )
            else:
                result = await db.execute(select(Function).filter_by(type=type))
            return [
                FunctionModel.model_validate(function)
                for function in result.scalars().all()
            ]
    ...
    async def get_global_action_functions(
        ...
        async with get_async_db_context(db) as db:
            result = await db.execute(
                select(Function).filter_by(
                    type="action", is_active=True, is_global=True
                )
            )
            return [
                FunctionModel.model_validate(function)
    ...
    async def get_function_valves_by_ids(
        ...
        try:
            async with get_async_db_context(db) as db:
                result = await db.execute(
                    select(Function.id, Function.valves).filter(Function.id.in_(ids))
                )
    ...
    async def deactivate_all_functions(
        self, db: AsyncSession | None = None
    ) -> bool | None:
        async with get_async_db_context(db) as db:
            try:
