...
class NoteTable:
    async def _get_access_grants(
        self, note_id: str, db: AsyncSession | None = None
    ...
    async def _to_note_model(
        ...
        note_data["access_grants"] = (
            ...
            else await self._get_access_grants(note_data["id"], db=db)
        )
        ...
        async with get_async_db_context(db) as db:
            ...
            if filter:
                ...
                if query_key:
                    ...
                    normalized_words = [
                        w.replace("-", "") for w in search_words if w.replace("-", "")
                    ]
    ...
    async def get_notes_by_user_id(
        ...
        async with get_async_db_context(db) as db:
            ...
            stmt = select(Note).order_by(Note.updated_at.desc())
            stmt = self._has_permission(
                db, stmt, {"user_id": user_id, "group_ids": user_group_ids}, permission
            )
        ...
        try:
            async with get_async_db_context(db) as db:
                ...
                pin_result = await db.execute(
                    select(PinnedNote).filter_by(user_id=user_id, note_id=id)
                )
                pinned_note = pin_result.scalars().first()

                if pinned_note:
                    await db.execute(
                        delete(PinnedNote).filter_by(user_id=user_id, note_id=id)
                    )
                else:
                    new_pin = PinnedNote(
                        id=str(uuid.uuid4()),
    ...
    async def get_pinned_note_ids(
        self, user_id: str, db: AsyncSession | None = None
    ) -> list[str]:
        async with get_async_db_context(db) as db:
            result = await db.execute(
                select(PinnedNote.note_id).filter_by(user_id=user_id)
            )
