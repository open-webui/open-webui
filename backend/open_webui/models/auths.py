...
class AuthsTable:
    ...
    async def authenticate_user(self, email: str, password: str) -> Optional[UserModel]:
        ...
        try:
            async with get_db() as db:
                auth = await db.query(Auth).filter_by(id=user.id, active=True).first()
                if auth:
    ...
    async def authenticate_user_by_email(self, email: str) -> Optional[UserModel]:
        ...
        try:
            async with get_db() as db:
                auth = await db.query(Auth).filter_by(email=email, active=True).first()
                if auth:
    ...
    async def update_email_by_id(self, id: str, email: str) -> bool:
        try:
            async with get_db() as db:
                result = await db.query(Auth).filter_by(id=id).update({"email": email})
                await db.commit()
                return True if result == 1 else False
        except Exception:
            return False
    ...
    async def delete_auth_by_id(self, id: str) -> bool:
        try:
            async with get_db() as db:
                ...
                if result:
                    await db.query(Auth).filter_by(id=id).delete()
                    await db.commit()
