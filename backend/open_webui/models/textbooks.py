...
class TextbookSection(Base):
    ...
    title = Column(
        Text, nullable=False
    )  # e.g., "Part A. 상미분방정식 (Ordinary Differential Equations)"
    ...
    chapters = relationship(
        "TextbookChapter", back_populates="section", cascade="all, delete-orphan"
    )
...
class TextbookChapter(Base):
    ...
    section_id = Column(
        String, ForeignKey("textbook_section.id", ondelete="CASCADE"), nullable=False
    )
    title = Column(
        Text, nullable=False
    )  # e.g., "1. 1계 상미분방정식 (First-Order ODEs)"
    subtitle = Column(
        Text, nullable=True
    )  # e.g., "1계 미분방정식의 기본 개념과 해법을 학습합니다."
    order = Column(Integer, default=0)  # 정렬 순서
    is_active = Column(Boolean, default=True)
    ...
    questions = relationship(
        "TextbookQuestion", back_populates="chapter", cascade="all, delete-orphan"
    )
...
class TextbookQuestion(Base):
    ...
    chapter_id = Column(
        String, ForeignKey("textbook_chapter.id", ondelete="CASCADE"), nullable=False
    )
...
class TextbookSections:
    ...
    def get_all() -> List[TextbookSectionModel]:
        with get_db() as db:
            sections = (
                ...
                .filter(TextbookSection.is_active == True)
                .order_by(TextbookSection.order)
                .all()
            )
            ...
            for section in sections:
                ...
                for chapter in chapters:
                    ...
                    chapter_responses.append(
                        TextbookChapterResponse(
                            ...
                            questions=[
                                TextbookQuestionModel.model_validate(q)
                                for q in questions
                            ],
                        )
                    )
                ...
                result.append(
                    TextbookSectionResponse(
                        id=section.id, title=section.title, chapters=chapter_responses
                    )
                )
    ...
    def get_by_id(section_id: str) -> Optional[TextbookSectionModel]:
        with get_db() as db:
            section = (
                db.query(TextbookSection)
                .filter(TextbookSection.id == section_id)
                .first()
            )
    ...
    @staticmethod
    def update(
        section_id: str, form: TextbookSectionUpdateForm
    ) -> Optional[TextbookSectionModel]:
        with get_db() as db:
            section = (
                db.query(TextbookSection)
                .filter(TextbookSection.id == section_id)
...
class TextbookChapters:
    ...
    def get_all() -> List[TextbookChapterModel]:
        with get_db() as db:
            chapters = (
                db.query(TextbookChapter)
                .filter(TextbookChapter.is_active == True)
                .order_by(TextbookChapter.order)
                .all()
            )
    ...
    def get_by_id(chapter_id: str) -> Optional[TextbookChapterModel]:
        with get_db() as db:
            chapter = (
                db.query(TextbookChapter)
                .filter(TextbookChapter.id == chapter_id)
                .first()
            )
    ...
    @staticmethod
    def update(
        chapter_id: str, form: TextbookChapterUpdateForm
    ) -> Optional[TextbookChapterModel]:
        with get_db() as db:
            chapter = (
                db.query(TextbookChapter)
                .filter(TextbookChapter.id == chapter_id)
    ...
    def get_rag_store(chapter_id: str) -> Optional[str]:
        ...
        with get_db() as db:
            chapter = (
                db.query(TextbookChapter)
                .filter(
                    TextbookChapter.id == chapter_id, TextbookChapter.is_active == True
                )
                .first()
            )
    ...
    def get_all_rag_store_mappings() -> dict:
        ...
        with get_db() as db:
            ...
            return {
                c.id: {"store_name": c.rag_store_name, "display_name": c.title}
                for c in chapters
            }

    @staticmethod
    def set_rag_store(
        chapter_id: str, store_name: Optional[str]
    ) -> Optional[TextbookChapterModel]:
        """챕터에 RAG Store 설정/해제"""
        with get_db() as db:
            chapter = (
    ...
    def clear_rag_store_by_name(store_name: str) -> List[str]:
        ...
        with get_db() as db:
            chapters = (
                db.query(TextbookChapter)
                .filter(TextbookChapter.rag_store_name == store_name)
                .all()
            )
...
class TextbookQuestions:
    ...
    def get_all() -> List[TextbookQuestionModel]:
        with get_db() as db:
            questions = (
                db.query(TextbookQuestion)
                .filter(TextbookQuestion.is_active == True)
                .order_by(TextbookQuestion.order)
                .all()
            )
    ...
    def get_by_id(question_id: str) -> Optional[TextbookQuestionModel]:
        with get_db() as db:
            question = (
                db.query(TextbookQuestion)
                .filter(TextbookQuestion.id == question_id)
                .first()
            )
    ...
    def get_recommendations(
        ...
        with get_db() as db:
            query = (
                db.query(TextbookQuestion, TextbookChapter, TextbookSection)
                .join(
                    TextbookChapter, TextbookQuestion.chapter_id == TextbookChapter.id
                )
                .join(TextbookSection, TextbookChapter.section_id == TextbookSection.id)
                .filter(
                    TextbookQuestion.is_active == True,
                    TextbookChapter.is_active == True,
    ...
    @staticmethod
    def update(
        question_id: str, form: TextbookQuestionUpdateForm
    ) -> Optional[TextbookQuestionModel]:
        with get_db() as db:
            question = (
                db.query(TextbookQuestion)
                .filter(TextbookQuestion.id == question_id)
...
INITIAL_SECTIONS = [
    {
        "id": "part-a",
        "title": "Part A. 상미분방정식 (Ordinary Differential Equations)",
        "order": 1,
    },
    {
        "id": "part-b",
        "title": "Part B. 선형대수, 벡터 미적분 (Linear Algebra, Vector Calculus)",
        "order": 2,
    },
    {
        "id": "part-c",
        "title": "Part C. 푸리에 해석, 편미분방정식 (Fourier Analysis, PDEs)",
        "order": 3,
    },
    ...
    {
        "id": "part-f",
        "title": "Part F. 최적화, 그래프 (Optimization, Graphs)",
        "order": 6,
    },
    {
        "id": "part-g",
        "title": "Part G. 확률, 통계 (Probability, Statistics)",
        "order": 7,
    },
...
INITIAL_CHAPTERS = [
    ...
    },
    {
        "id": "ch-4",
        "section_id": "part-a",
        "title": "4. 상미분방정식의 연립계, 위상평면, 정성적 방법 (Systems of ODEs, Phase Plane, Qualitative Methods)",
        "subtitle": "연립 미분방정식과 위상평면 분석을 학습합니다.",
        "order": 4,
    },
    {
        "id": "ch-5",
        "section_id": "part-a",
        "title": "5. 급수해법, 특수함수 (Series Solutions of ODEs, Special Functions)",
        "subtitle": "급수를 이용한 미분방정식의 해법과 특수함수를 다룹니다.",
        "order": 5,
    },
    {
        "id": "ch-6",
        "section_id": "part-a",
        "title": "6. 라플라스 변환 (Laplace Transforms)",
        "subtitle": "라플라스 변환을 이용한 미분방정식 해법을 학습합니다.",
        "order": 6,
    },
    # Part B: Linear Algebra, Vector Calculus (Chapters 7-10)
    {
        "id": "ch-7",
        "section_id": "part-b",
        "title": "7. 선형대수: 행렬, 벡터, 선형시스템 (Linear Algebra: Matrices, Vectors, Linear Systems)",
        "subtitle": "행렬과 벡터의 기본 개념, 선형시스템을 이해합니다.",
        "order": 7,
    },
    {
        "id": "ch-8",
        "section_id": "part-b",
        "title": "8. 행렬 고유값 문제 (Matrix Eigenvalue Problems)",
    ...
    },
    {
        "id": "ch-9",
        "section_id": "part-b",
        "title": "9. 벡터 미분학: 기울기, 발산, 회전 (Vector Differential Calculus: Grad, Div, Curl)",
    ...
    },
    {
        "id": "ch-10",
        "section_id": "part-b",
        "title": "10. 벡터 적분학, 적분정리 (Vector Integral Calculus, Integral Theorems)",
        "subtitle": "선적분, 면적분과 적분정리를 학습합니다.",
        "order": 10,
    },
    # Part C: Fourier Analysis, PDEs (Chapters 11-12)
    {
        "id": "ch-11",
        "section_id": "part-c",
        "title": "11. 푸리에 해석 (Fourier Analysis)",
        "subtitle": "푸리에 급수와 푸리에 변환을 학습합니다.",
        "order": 11,
    },
    {
        "id": "ch-12",
        "section_id": "part-c",
        "title": "12. 편미분방정식 (Partial Differential Equations)",
    ...
    # Part D: Complex Analysis (Chapters 13-18)
    {
        "id": "ch-13",
        "section_id": "part-d",
        "title": "13. 복소수와 복소함수 (Complex Numbers and Functions)",
        "subtitle": "복소수의 기본 개념과 복소함수를 이해합니다.",
        "order": 13,
    },
    {
        "id": "ch-14",
        "section_id": "part-d",
        "title": "14. 복소적분 (Complex Integration)",
        "subtitle": "복소평면에서의 적분을 학습합니다.",
        "order": 14,
    },
    {
        "id": "ch-15",
        "section_id": "part-d",
        "title": "15. 거듭제곱급수, 테일러급수 (Power Series, Taylor Series)",
        "subtitle": "복소함수의 거듭제곱급수 전개를 다룹니다.",
        "order": 15,
    },
    {
        "id": "ch-16",
        "section_id": "part-d",
        "title": "16. 로랑급수, 유수 (Laurent Series, Residues)",
        "subtitle": "로랑급수와 유수정리를 학습합니다.",
        "order": 16,
    },
    {
        "id": "ch-17",
        "section_id": "part-d",
        "title": "17. 등각사상 (Conformal Mapping)",
        "subtitle": "복소평면에서의 등각사상을 이해합니다.",
        "order": 17,
    },
    {
        "id": "ch-18",
        "section_id": "part-d",
        "title": "18. 포텐셜 이론의 응용 (Applications to Potential Theory)",
        "subtitle": "복소해석의 포텐셜 이론 응용을 학습합니다.",
        "order": 18,
    },
    # Part E: Numeric Analysis (Chapters 19-21)
    {
        "id": "ch-19",
        "section_id": "part-e",
        "title": "19. 수치해석 일반 (Numerics in General)",
        "subtitle": "수치해석의 기본 개념과 오차 분석을 학습합니다.",
        "order": 19,
    },
    {
        "id": "ch-20",
        "section_id": "part-e",
        "title": "20. 수치 선형대수 (Numerical Linear Algebra)",
        "subtitle": "선형시스템의 수치적 해법을 다룹니다.",
        "order": 20,
    },
    {
        "id": "ch-21",
        "section_id": "part-e",
        "title": "21. 상미분방정식과 편미분방정식의 수치해법 (Numerics for ODEs and PDEs)",
        "subtitle": "미분방정식의 수치적 해법을 학습합니다.",
        "order": 21,
    },
    # Part F: Optimization, Graphs (Chapters 22-23)
    {
        "id": "ch-22",
        "section_id": "part-f",
        "title": "22. 비제약 최적화, 선형계획법 (Unconstrained Optimization, Linear Programming)",
        "subtitle": "최적화 문제와 선형계획법을 학습합니다.",
        "order": 22,
    },
    {
        "id": "ch-23",
        "section_id": "part-f",
        "title": "23. 그래프와 조합 최적화 (Graphs and Combinatorial Optimization)",
        "subtitle": "그래프 이론과 조합 최적화를 다룹니다.",
        "order": 23,
    },
    # Part G: Probability, Statistics (Chapters 24-25)
    {
        "id": "ch-24",
        "section_id": "part-g",
        "title": "24. 데이터 분석, 확률론 (Data Analysis, Probability Theory)",
        "subtitle": "데이터 분석과 확률론의 기초를 학습합니다.",
        "order": 24,
    },
    {
        "id": "ch-25",
        "section_id": "part-g",
        "title": "25. 수리통계학 (Mathematical Statistics)",
        "subtitle": "수리통계학의 주요 개념을 다룹니다.",
        "order": 25,
    },
    # General
    {
...
INITIAL_QUESTIONS = [
    ...
    # Chapter 4
    {
        "id": "q6",
        "chapter_id": "ch-4",
        "question": "위상평면에서 임계점의 안정성은 어떻게 분석하나요?",
        "order": 1,
    },
    # Chapter 5
    {
        "id": "q7",
        "chapter_id": "ch-5",
        "question": "프로베니우스 방법(Frobenius method)을 설명해주세요.",
        "order": 1,
    },
    {
        "id": "q8",
        "chapter_id": "ch-5",
        "question": "베셀 함수(Bessel functions)의 정의와 응용은?",
        "order": 2,
    },
    # Chapter 6
    {
        "id": "q9",
        "chapter_id": "ch-6",
        "question": "라플라스 변환의 기본 성질과 공식은 무엇인가요?",
        "order": 1,
    },
    {
        "id": "q10",
        "chapter_id": "ch-6",
        "question": "라플라스 변환을 이용해 미분방정식을 푸는 과정을 설명해주세요.",
        "order": 2,
    },
    # Chapter 7
    {
    ...
    # Chapter 10
    {
        "id": "q17",
        "chapter_id": "ch-10",
        "question": "그린 정리(Green's Theorem)를 설명해주세요.",
        "order": 1,
    },
    {
        "id": "q18",
        "chapter_id": "ch-10",
        "question": "스토크스 정리(Stokes' Theorem)와 발산 정리의 관계는?",
        "order": 2,
    },
    # Chapter 11
    {
    ...
    # Chapter 15
    {
        "id": "q26",
        "chapter_id": "ch-15",
        "question": "복소함수의 테일러 급수 전개는 어떻게 하나요?",
        "order": 1,
    },
    # Chapter 16
    {
    ...
    },
    {
        "id": "q28",
        "chapter_id": "ch-16",
        "question": "로랑 급수와 테일러 급수의 차이점은 무엇인가요?",
        "order": 2,
    },
    # Chapter 17
    {
        "id": "q29",
        "chapter_id": "ch-17",
        "question": "등각사상(conformal mapping)이란 무엇인가요?",
        "order": 1,
    },
    # Chapter 18
    {
        "id": "q30",
        "chapter_id": "ch-18",
        "question": "복소해석이 유체역학에 어떻게 응용되나요?",
        "order": 1,
    },
    # Chapter 19
    {
        "id": "q31",
        "chapter_id": "ch-19",
        "question": "수치해석에서 오차의 종류와 분석 방법은?",
        "order": 1,
    },
    {
        "id": "q32",
        "chapter_id": "ch-19",
        "question": "뉴턴-랩슨 방법(Newton-Raphson method)을 설명해주세요.",
        "order": 2,
    },
    # Chapter 20
    {
        "id": "q33",
        "chapter_id": "ch-20",
        "question": "가우스 소거법(Gaussian elimination)의 원리는?",
        "order": 1,
    },
    {
        "id": "q34",
        "chapter_id": "ch-20",
        "question": "LU 분해(LU decomposition)를 설명해주세요.",
        "order": 2,
    },
    # Chapter 21
    {
        "id": "q35",
        "chapter_id": "ch-21",
        "question": "오일러 방법(Euler method)과 룽게-쿠타 방법의 차이점은?",
        "order": 1,
    },
    {
        "id": "q36",
        "chapter_id": "ch-21",
        "question": "유한차분법(finite difference method)을 설명해주세요.",
        "order": 2,
    },
    # Chapter 22
    {
        "id": "q37",
        "chapter_id": "ch-22",
        "question": "경사하강법(gradient descent)의 원리를 설명해주세요.",
        "order": 1,
    },
    {
        "id": "q38",
        "chapter_id": "ch-22",
        "question": "심플렉스 방법(Simplex method)이란 무엇인가요?",
        "order": 2,
    },
    # Chapter 23
    {
        "id": "q39",
        "chapter_id": "ch-23",
        "question": "그래프 이론에서 최단 경로 알고리즘은 어떤 것이 있나요?",
        "order": 1,
    },
    {
        "id": "q40",
        "chapter_id": "ch-23",
        "question": "최소 신장 트리(minimum spanning tree)를 구하는 방법은?",
        "order": 2,
    },
    # Chapter 24
    {
        "id": "q41",
        "chapter_id": "ch-24",
        "question": "베이즈 정리(Bayes' Theorem)를 설명해주세요.",
        "order": 1,
    },
    {
        "id": "q42",
        "chapter_id": "ch-24",
        "question": "정규분포(normal distribution)의 특성은 무엇인가요?",
        "order": 2,
    },
    # Chapter 25
    {
        "id": "q43",
        "chapter_id": "ch-25",
        "question": "최대우도추정(MLE)의 원리를 설명해주세요.",
        "order": 1,
    },
    {
        "id": "q44",
        "chapter_id": "ch-25",
        "question": "가설검정(hypothesis testing)의 절차는 어떻게 되나요?",
        "order": 2,
    },
]
