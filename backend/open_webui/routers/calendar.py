...
async def get_events(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid date format. Use ISO 8601 (e.g. 2026-04-01T00:00:00)",
        )
    ...
    for event in events:
        ...
        if event_dict.get("rrule"):
            ...
            for inst in instances:
                expanded.append(
                    CalendarEventUserResponse(**{**inst, "user": event.user})
                )
        else:
    ...
    if await _user_has_automations(request, user) and (
        ...
        try:
            ...
            for auto in active_automations:
                ...
                instances = expand_recurring_event(
                    virtual, rrule_start, end_ns, tz=user.timezone
                )
                for inst in instances:
...
async def get_event(
    request: Request, event_id: str, user: UserModel = Depends(get_verified_user)
):
    await check_calendar_permission(request, user)
...
async def rsvp_event(
    ...
    result = await CalendarEventAttendees.update_rsvp(
        event_id, user.id, form_data.status
    )
    if not result:
...
async def get_calendar_by_id(
    request: Request, calendar_id: str, user: UserModel = Depends(get_verified_user)
):
    await check_calendar_permission(request, user)
...
async def update_calendar(
    ...
    if (
        form_data.access_grants is not None
        and cal.user_id != user.id
        and user.role != "admin"
    ):
...
async def delete_calendar(
    ...
    if calendar_id == SCHEDULED_TASKS_CALENDAR_ID:
        raise HTTPException(
            status_code=400, detail="System calendars cannot be deleted"
        )
    ...
    if cal.is_default:
        raise HTTPException(
            status_code=400, detail="Default calendar cannot be deleted"
        )
