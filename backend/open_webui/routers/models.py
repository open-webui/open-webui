...
async def get_models(
    ...
    result = await Models.search_models(
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            existing_models = {
                model.id: model
                for model in (
                    await Models.get_models_by_ids(model_ids, db=db)
                    if model_ids
                    else []
                )
            }
                    ...
                    if existing_model:
                        ...
                        if "access_grants" in model_data:
                            updated_model.access_grants = (
                                await filter_allowed_access_grants(
                                    request.app.state.config.USER_PERMISSIONS,
                                    user.id,
                                    user.role,
                                    updated_model.access_grants,
...
async def update_model_by_id(
    ...
    model = await Models.update_model_by_id(
