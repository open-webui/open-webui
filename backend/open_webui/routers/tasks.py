...
async def update_task_config(
    ...
    request.app.state.config.ENABLE_VOICE_MODE_PROMPT = (
        form_data.ENABLE_VOICE_MODE_PROMPT
    )
...
async def generate_image_prompt(
    ...
    content = await image_prompt_generation_template(
        template, form_data["messages"], user
    )
...
async def generate_autocompletion(
    ...
    if request.app.state.config.AUTOCOMPLETE_GENERATION_INPUT_MAX_LENGTH > 0:
        if (
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=ERROR_MESSAGES.INPUT_TOO_LONG(
                    request.app.state.config.AUTOCOMPLETE_GENERATION_INPUT_MAX_LENGTH
                ),
            )
    ...
    content = await autocomplete_generation_template(
