...
async def channel_has_access(
    ...
    if (
        ...
        and has_public_write_access_grant(channel.access_grants)
    ):
        return True
...
async def get_channels(
    ...
    for channel in channels:
        ...
        channel_member = await Channels.get_member_by_channel_and_user_id(
            channel.id, user.id, db=db
        )
        unread_count = (
            await Messages.get_unread_message_count(
        ...
        if channel.type == "dm":
            user_ids = [
                member.user_id
                for member in await Channels.get_members_by_channel_id(
                    channel.id, db=db
                )
            ]
            users = [
...
async def get_dm_channel_by_user_id(
    ...
    await check_channels_access(request, user)
    try:
        existing_channel = await Channels.get_dm_channel_by_user_ids(
        ...
        if existing_channel:
            ...
            await Channels.update_member_active_status(
        ...
        return ChannelFullResponse(
            **{
                **channel.model_dump(),
                "user_ids": user_ids,
                "users": users,
                "is_manager": await Channels.is_user_channel_manager(
                    channel.id, user.id, db=db
                ),
                "write_access": True,
...
async def get_channel_members_by_id(
    ...
    if channel.type == "dm":
        ...
        return {
            "users": [
                UserModelResponse(**u.model_dump(), is_active=Users.is_active(u))
...
async def remove_members_by_id(
    ...
    try:
        deleted = await Channels.remove_members_from_channel(
...
async def get_channel_messages(
    ...
    if channel.type in ["group", "dm"]:
        ...
        channel_member = await Channels.join_channel(
            id, user.id, db=db
    ...
    fetched_users = {
        u.id: u for u in await Users.get_users_by_user_ids(user_ids, db=db)
    }
...
async def send_notification(request, channel, message, active_user_ids, db=None):
    ...
    for u in users:
        if (u.id not in active_user_ids) and await Channels.is_user_channel_member(
            channel.id, u.id, db=db
        ):
            if enable_user_webhooks and u.settings:
                webhook_url = u.settings.ui.get("notifications", {}).get(
                    "webhook_url", None
                )
                if webhook_url:
                    await post_webhook(
...
async def model_response_handler(request, channel, message, user, db=None):
    MODELS = {
        model["id"]: model
        for model in await get_filtered_models(
            await get_all_models(request, user=user), user
        )
    }
    ...
    for mention in model_mentions.values():
        ...
        if model:
            try:
                ...
                message_users = {
                    user.id: user
                    for user in await Users.get_users_by_user_ids(user_ids, db=db)
                }
...
async def new_message_handler(
    ...
    try:
        message = await Messages.insert_new_message(
            form_data, channel.id, user.id, db=db
        )
        if message:
            if channel.type in ["group", "dm"]:
            ...
            if message.parent_id:
                ...
                parent_message = await Messages.get_message_by_id(
                    message.parent_id, db=db
                )
...
async def get_channel_message(
    ...
    return MessageResponse(
        **{
            **message.model_dump(),
            "user": (
                UserNameResponse(**message_user.model_dump()) if message_user else None
            ),
        }
    )
...
async def pin_channel_message(
    ...
    try:
        await Messages.update_is_pinned_by_id(
            message_id, form_data.is_pinned, user.id, db=db
        )
        message = await Messages.get_message_by_id(message_id, db=db)
        message_user = await Users.get_user_by_id(message.user_id, db=db)
        return MessageUserResponse(
            **{
                **message.model_dump(),
                "user": (
                    UserNameResponse(**message_user.model_dump())
                    if message_user
                    else None
                ),
            }
...
async def get_channel_thread_messages(
    ...
    message_list = await Messages.get_messages_by_parent_id(
...
async def add_reaction_to_message(
    ...
    try:
        await Messages.add_reaction_to_message(
...
async def get_channel_webhooks(
    ...
    if (
        not await Channels.is_user_channel_manager(channel.id, user.id, db=db)
        and user.role != "admin"
    ):
        raise HTTPException(
