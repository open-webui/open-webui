    ...
    try:
        ...
        if hasattr(tools_module, "on_valves_updated"):
            try:
                ...
                log.exception(
                    f"Error calling on_valves_updated for tool {id}: {callback_error}"
                )
    ...
    if tools:
        ...
        if hasattr(tools_module, "UserValves"):
            ...
            try:
                ...
                if hasattr(tools_module, "on_valves_updated"):
                    try:
                        tools_module.on_valves_updated(
                            user_valves.model_dump(), user_id=user.id
                        )
                    except Exception as callback_error:
                        log.exception(
                            f"Error calling on_valves_updated for tool {id} user {user.id}: {callback_error}"
                        )
