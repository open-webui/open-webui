...
async def update_embedding_config(
    ...
    try:
        ...
        request.app.state.config.RAG_EMBEDDING_MODEL = (
            form_data.RAG_EMBEDDING_MODEL.strip()
        )
    ...
    if form_data.web is not None:
        ...
        if form_data.web.BRAVE_SEARCH_CONTEXT_TOKENS is not None:
            request.app.state.config.BRAVE_SEARCH_CONTEXT_TOKENS = (
                form_data.web.BRAVE_SEARCH_CONTEXT_TOKENS
            )
    ...
    if file:
        try:
            ...
            if request.app.state.config.BYPASS_EMBEDDING_AND_RETRIEVAL:
                await Files.update_file_data_by_id(
                    file.id, {"status": "completed"}, db=db
                )
...
async def _validate_collection_access(
    ...
    allowed = await filter_accessible_collections(
        requested, user, access_type=access_type
    )
    ...
    try:
        if request.app.state.config.ENABLE_RAG_HYBRID_SEARCH and (
            ...
            collection_results[form_data.collection_name] = (
                await ASYNC_VECTOR_DB_CLIENT.get(
                    collection_name=form_data.collection_name
                )
            )
...
async def delete_entries_from_collection(
    ...
    try:
        if await ASYNC_VECTOR_DB_CLIENT.has_collection(
            collection_name=form_data.collection_name
        ):
            file = await Files.get_file_by_id(form_data.file_id, db=db)
            if not file:
