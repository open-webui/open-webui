...
async def get_knowledge_by_id(
    ...
    if knowledge:
        ...
        if (
            ...
            file_ids = KnowledgeFiles.get_file_ids_by_knowledge_id(id, limit=limit, offset=offset)
            files = Files.get_file_metadatas_by_ids(file_ids)
...
def add_file_to_knowledge_by_id(
    ...
    existing_file_ids = KnowledgeFiles.get_file_ids_by_knowledge_id(id)
    if form_data.file_id in existing_file_ids:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ERROR_MESSAGES.DEFAULT("file_id"),
        )

    # Add file to knowledge_file table
    knowledge_file = KnowledgeFiles.insert_knowledge_file(
        knowledge_id=id,
...
def remove_file_from_knowledge_by_id(
    ...
    # Remove file from knowledge_file table
    success = KnowledgeFiles.delete_knowledge_file(id, form_data.file_id)
...
async def reset_knowledge_by_id(id: str, user=Depends(get_verified_user)):
    ...
    # Delete all file associations for this knowledge base
    KnowledgeFiles.delete_knowledge_files_by_knowledge_id(id)
...
def add_files_to_knowledge_batch(
    ...
    for file_id in successful_file_ids:
        if file_id not in existing_file_ids:
            KnowledgeFiles.insert_knowledge_file(
                knowledge_id=id,
                file_id=file_id,
                user_id=user.id
            )
