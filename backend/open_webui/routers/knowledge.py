...
async def reindex_knowledge_files(
    ...
    for knowledge_base in knowledge_bases:
        try:
            ...
            try:
                if _vector_db_client().has_collection(collection_name=knowledge_base.id):
                    _vector_db_client().delete_collection(collection_name=knowledge_base.id)
            except Exception as e:
...
def update_file_from_knowledge_by_id(
    ...
    _vector_db_client().delete(collection_name=knowledge.id, filter={'file_id': form_data.file_id})
...
def remove_file_from_knowledge_by_id(
    ...
    if delete_file:
        try:
            ...
            if _vector_db_client().has_collection(collection_name=file_collection):
                _vector_db_client().delete_collection(collection_name=file_collection)
        except Exception as e:
            log.debug('This was most likely caused by bypassing embedding processing')
            log.debug(e)
            pass
...
async def delete_knowledge_by_id(id: str, user=Depends(get_verified_user), db: Session = Depends(get_session)):
    ...
    # Clean up vector DB
    try:
        _vector_db_client().delete_collection(collection_name=id)
    except Exception as e:
        log.debug(e)
        pass
