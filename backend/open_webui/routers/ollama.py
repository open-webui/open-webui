...
async def show_model_info(
    ...
    await check_model_access(
        user, await Models.get_model_by_id(model), BYPASS_MODEL_ACCESS_CONTROL
    )
...
async def embed(
    ...
    # Enforce per-model access control
    await check_model_access(
        user, await Models.get_model_by_id(form_data.model), BYPASS_MODEL_ACCESS_CONTROL
    )
        ...
        async with session.get(
            ...
            with open(file_path, "ab+") as file:
                ...
                if done:
                    ...
                    async with aiohttp.ClientSession(
                        timeout=blob_timeout, trust_env=True
                    ) as blob_session:
...
async def upload_model(
    ...
    async def file_process_stream():
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                timeout=upload_timeout, trust_env=True
            ) as upload_session:
