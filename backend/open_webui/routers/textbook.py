...
async def get_textbook():
    ...
    for section in sections_data:
        subsections = [
            Subsection(
                id=chapter.id, title=chapter.title, subtitle=chapter.subtitle or ""
            )
            for chapter in section.chapters
        ]
        sections.append(
            Section(id=section.id, title=section.title, subsections=subsections)
        )
...
async def get_section(section_id: str):
    ...
    subsections = [
        Subsection(id=chapter.id, title=chapter.title, subtitle=chapter.subtitle or "")
        for chapter in chapters
    ]

    return Section(id=section.id, title=section.title, subsections=subsections)


@router.get(
    "/sections/{section_id}/subsections/{subsection_id}", response_model=Subsection
)
...
async def get_recommendations(
    k: int = Query(
        default=5, ge=1, le=20, description="Number of recommendations to return"
    ),
    section_id: Optional[str] = Query(default=None, description="Filter by section ID"),
    subsection_id: Optional[str] = Query(
        default=None, description="Filter by subsection ID (chapter ID)"
    ),
    ...
    questions = TextbookQuestions.get_recommendations(
        k=k, section_id=section_id, chapter_id=subsection_id, shuffle=shuffle
    )
...
async def get_stats_by_chapter(
    ...
    include_questions: bool = Query(
        default=True, description="Include question list in response"
    ),
    max_questions_per_chapter: int = Query(
        default=10, ge=1, le=100, description="Max questions per chapter"
    ),
):
    """
    ...
    for chapter_id, questions in chapter_questions.items():
        ...
        limited_questions = (
            sorted_questions[:max_questions_per_chapter] if include_questions else []
        )
    ...
    return ChapterStatsResponse(
        period_days=days, total_questions=total_questions, chapters=chapters
    )
...
async def admin_create_section(form: TextbookSectionForm, user=Depends(get_admin_user)):
    """
...
async def admin_update_section(
    section_id: str, form: TextbookSectionUpdateForm, user=Depends(get_admin_user)
):
    """
...
async def admin_delete_section(section_id: str, user=Depends(get_admin_user)):
    """
...
async def admin_create_chapter(form: TextbookChapterForm, user=Depends(get_admin_user)):
    """
...
async def admin_update_chapter(
    chapter_id: str, form: TextbookChapterUpdateForm, user=Depends(get_admin_user)
):
    """
...
async def admin_delete_chapter(chapter_id: str, user=Depends(get_admin_user)):
    """
...
async def admin_create_question(
    form: TextbookQuestionForm, user=Depends(get_admin_user)
):
    """
...
async def admin_update_question(
    question_id: str, form: TextbookQuestionUpdateForm, user=Depends(get_admin_user)
):
    """
