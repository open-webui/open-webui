...
async def _try_provider_candidate(
    ...
    if cache_entry and cache_entry.get('pipeline') and cache_entry.get('urlIdx') == idx:
    ...
    requested_model = attempt_payload.get('model')
    ...
    is_streaming_request = bool(attempt_payload.get('stream', False))
    ...
    try:
        ...
        if r.status >= 400:
            ...
            # Non-retryable failure: surface it to the provider-failure event
            # stream (mirrors the SSE error branch) before returning to client.
            await publish_model_provider_request_failed(
                request,
                actor=user,
                provider='openai-compatible',
