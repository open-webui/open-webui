...
async def export_chat_stats(
    ...
    try:
        ...
        if stream:
            ...
            chat_stats_export_list, total = await calculate_chat_stats(
                user.id, skip, limit, filter
            )
...
async def get_chats_by_folder_id(
    ...
    return [
        ...
        for chat in await Chats.get_chats_by_folder_ids_and_user_id(
...
async def update_shared_chat_access_by_id(
    ...
    await AccessGrants.set_access_grants(
        "shared_chat", id, form_data.access_grants, db=db
    )
...
async def delete_tag_by_id_and_tag_name(
    ...
    if chat:
        ...
        if (
            await Chats.count_chats_by_tag_name_and_user_id(
                form_data.name, user.id, db=db
            )
            == 0
        ):
            await Tags.delete_tag_by_name_and_user_id(form_data.name, user.id, db=db)
