...
from open_webui.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES, GOOGLE_SHEET_CREDENTIALS
...
@cached(TTLCache(maxsize=300,ttl=300))
def get_allowed_users() -> set:
    sheet = SA.open("AMLLM Access Control")
    wk = sheet.worksheet("Sheet1")

    allowed_users = set([row["Emails"] for row in wk.get_all_records()])
...
async def signup(request: Request, response: Response, form_data: SignupForm):
    ...
    if form_data.email.lower() not in allowed_users:
