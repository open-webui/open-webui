...
def _validate_slug(slug: str):
    if len(slug) < 2 or len(slug) > 50:
        raise HTTPException(400, detail='Slug must be between 2 and 50 characters')
    if not SLUG_PATTERN.match(slug):
        raise HTTPException(
            400,
            detail='Slug must contain only lowercase letters, numbers, and hyphens, and must start/end with alphanumeric',
        )
    if slug in BUILTIN_OAUTH_PROVIDER_SLUGS:
        raise HTTPException(
            400,
            detail=f"Slug '{slug}' conflicts with a built-in OAuth provider",
        )
...
async def get_custom_oauth_providers(request: Request, user=Depends(get_admin_user)):
    providers = CUSTOM_OAUTH_PROVIDERS_CONFIG.value or []
...
async def create_custom_oauth_provider(
    ...
    providers = list(CUSTOM_OAUTH_PROVIDERS_CONFIG.value or [])
