...
async def get_model_history(
    ...
    history = await Feedbacks.get_model_evaluation_history(
...
async def get_feedback_model_ids(
...
async def export_all_feedbacks(
    ...
    if model_id:
        feedbacks = [
            f for f in feedbacks if f.data and f.data.get("model_id") == model_id
        ]
    return feedbacks
...
async def get_feedbacks(
    ...
    result = await Feedbacks.get_feedback_items(
...
async def get_feedback_by_id(
    ...
    if user.role == "admin":
        ...
        feedback = await Feedbacks.get_feedback_by_id_and_user_id(
...
async def update_feedback_by_id(
    ...
    if user.role == "admin":
        feedback = await Feedbacks.update_feedback_by_id(
            id=id, form_data=form_data, db=db
        )
    else:
