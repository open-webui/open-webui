...
# Because the REST API is Firecrawl-compatible, this provider mirrors the
# Firecrawl provider closely and simply targets fastCRW's base URL and routes.
...
def build_crw_headers(api_key: str | None) -> dict[str, str]:
    ...
    # Self-hosted fastCRW runs keyless by default; only send auth when a key is set.
    if api_key:
...
def search_crw(
    ...
    try:
        ...
        # `{"data": [{title, url, description}]}` list.
        data = response.get('data') or {}
