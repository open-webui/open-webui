...
class DatalabMarkerLoader:
    ...
    def _submit_request(self, filename: str, mime_type: str, headers: dict, form_data: dict) -> dict:
        request_params = {
            'filename': filename,
            'mime_type': mime_type,
            **form_data,
    ...
    def _poll_for_result(self, check_url: str, headers: dict) -> dict:
        for _ in range(300):  # Up to 10 minutes
            ...
            try:
                ...
                raw_body = poll_response.text if poll_response is not None else ''
    ...
    def _get_direct_result(self, result: dict) -> tuple[object, dict]:
        if 'output' in result:
        ...
        raise HTTPException(
            status.HTTP_502_BAD_GATEWAY,
            detail=(
                "Custom Marker endpoint returned success but no 'output' field found. "
                f'Available fields: {available_fields}. Expected either '
                "'request_check_url' for polling or 'output' field for direct response."
            ),
        )

    def _build_full_text(self, raw_content: object) -> str:
        output_format = cast(str, self.output_format).lower()
    ...
    def load(self) -> list[Document]:
        ...
        result = self._submit_request(filename, mime_type, headers, form_data)
        ...
        if check_url:
            poll_result = self._poll_for_result(check_url, headers)
            ...
            content_key = cast(str, self.output_format).lower()
            raw_content = poll_result.get(content_key)
            final_result = poll_result
        else:
            raw_content, final_result = self._get_direct_result(result)

        full_text = self._build_full_text(raw_content)
        ...
        file_ext = file_ext_map.get(cast(str, self.output_format).lower(), 'txt')
