...
def get_namespace_for_collection(
    ...
    See ``routers/retrieval.get_namespace_for_collection`` for the full
    rationale. The two implementations must stay in sync; this one is the
    copy used by ``retrieval/utils.py`` to avoid a circular import.

    For all other vector DBs return None.
    """
...
def build_loader_from_config(request):
    ...
    return Loader(
        ...
        # Local: forward chunking + Unstructured.io tunables so the unified loader
        # picks up the admin-configured chunk sizes and Unstructured strategies.
        CHUNK_SIZE=getattr(config, 'CHUNK_SIZE', 1000),
        CHUNK_OVERLAP=getattr(config, 'CHUNK_OVERLAP', 200),
        UNSTRUCTURED_STRATEGY=getattr(config, 'UNSTRUCTURED_STRATEGY', 'hi_res'),
        UNSTRUCTURED_CHUNKING_STRATEGY=getattr(config, 'UNSTRUCTURED_CHUNKING_STRATEGY', 'by_title'),
        UNSTRUCTURED_CLEANING_LEVEL=getattr(config, 'UNSTRUCTURED_CLEANING_LEVEL', 'standard'),
        UNSTRUCTURED_INCLUDE_METADATA=getattr(config, 'UNSTRUCTURED_INCLUDE_METADATA', True),
        UNSTRUCTURED_CLEAN_TEXT=getattr(config, 'UNSTRUCTURED_CLEAN_TEXT', True),
        UNSTRUCTURED_SEMANTIC_CHUNKING=getattr(config, 'UNSTRUCTURED_SEMANTIC_CHUNKING', True),
        UNSTRUCTURED_INFER_TABLE_STRUCTURE=getattr(config, 'UNSTRUCTURED_INFER_TABLE_STRUCTURE', False),
        UNSTRUCTURED_EXTRACT_IMAGES_IN_PDF=getattr(config, 'UNSTRUCTURED_EXTRACT_IMAGES_IN_PDF', False),
...
def query_doc(collection_name: str, query_embedding: list[float], k: int, user: UserModel = None):
    try:
        ...
        log.debug(f"query_doc:doc {collection_name}" + (f" in namespace '{namespace}'" if namespace else ""))
...
def get_doc(collection_name: str, user: UserModel = None):
    try:
        namespace = get_namespace_for_collection(collection_name)
        log.debug(f"get_doc:doc {collection_name}" + (f" in namespace '{namespace}'" if namespace else ""))
        result = VECTOR_DB_CLIENT.get(collection_name=collection_name, namespace=namespace)
...
async def query_collection_with_hybrid_search(
    ...
    results = []
    error = False
    # Prefetch every collection's contents once up front so the per-query/per-document
    # loop below can reuse them. Each fetch goes through asyncio.to_thread so we keep
    # upstream's concurrent fetch pattern while still passing namespace=... required by
    # the local Pinecone integration (the async client does not yet plumb namespace).
    log.debug(
    ...
    async def _fetch_collection(name: str):
        namespace = get_namespace_for_collection(name)
        if namespace:
            log.debug(
                "query_collection_with_hybrid_search:VECTOR_DB_CLIENT.get:collection %s in namespace '%s'",
                name,
                namespace,
            )
        try:
