...
async def query_collection(
    ...
    if not queries:
        log.warning(
            "query_collection: all queries were None or empty, returning empty results"
        )
...
async def filter_accessible_collections(
    ...
    for name in collection_names:
        if name == "knowledge-bases":
            ...
            if await has_access_to_file(
                file_id=file_id, access_type=access_type, user=user
            ):
                validated.add(name)
...
async def get_sources_from_items(
    ...
    for item in items:
        ...
        if query_result is None and collection_names:
            ...
            if user:
                collection_names = await filter_accessible_collections(
