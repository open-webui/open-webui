...
def generate_portkey_batch_embeddings(
    ...
    try:
        ...
        log.info(f"Processing embeddings for {len(texts)} text chunks in a single batch")
        ...
        while retry_count <= max_retries:
            try:
                ...
                if e.response.status_code == 429 and retry_count < max_retries and backoff:
...
def generate_embeddings(engine: str, model: str, text: Union[str, list[str]], backoff: bool, **kwargs):
    url = kwargs.get("url", "")
    key = kwargs.get("key", "")
    ...
    if engine == "ollama":
        ...
        if isinstance(text, list):
            embeddings = generate_portkey_batch_embeddings(model, text, url, key, user, backoff)
        else:
            embeddings = generate_portkey_batch_embeddings(model, [text], url, key, user, backoff)
    else:
    ...
    if isinstance(embeddings, EmbeddingResult):
        ...
        return embeddings.embeddings[0] if isinstance(text, str) else embeddings.embeddings
