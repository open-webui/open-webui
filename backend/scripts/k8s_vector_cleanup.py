...
def main():
    parser = argparse.ArgumentParser(description='CANChat Vector DB Cleanup for Kubernetes')
    ...
    if database_url:
        if database_url.startswith('postgresql://') or database_url.startswith('postgres://'):
            db_type = 'PostgreSQL'
            logger.info(f"🗄️  Database: {db_type} - {database_url.split('@')[1] if '@' in database_url else 'configured'}")
    ...
    try:
        ...
        try:
            ...
            logger.error("This could indicate database connectivity issues or missing credentials")
            return 1
        ...
        logger.info(f"🔍 Cleaning web search results older than {web_search_expiry_days} days...")
        if not args.dry_run:
            ...
            logger.info(f"✓ [DRY RUN] Would clean web searches older than {web_search_expiry_days} days")
        ...
        if not args.dry_run:
            ...
            logger.info(f"🎉 Cleanup completed successfully! Total items cleaned: {total_cleaned}")
        ...
        logger.error("Make sure the app is properly deployed and dependencies are available")
        return 1
