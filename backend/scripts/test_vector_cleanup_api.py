...
def test_endpoint(endpoint: str, method: str = "GET", data: Dict[Any, Any] = None) -> Dict[Any, Any]:
    ...
    try:
        if method == "GET":
            ...
            response = requests.post(f"{CANCHAT_URL}{endpoint}", headers=headers, json=data)
        else:
    ...
    for test in endpoints:
        ...
        if "error" in result:
            ...
            if "cleanup_result" in result:
                ...
                print(f"   🧹 Cleaned: {cleanup.get('collections_cleaned', 0)} collections, "
            ...
            if "cleanup_results" in result:
                ...
                print(f"   🧹 Orphaned: {orphaned.get('collections_cleaned', 0)} collections")
                print(f"   🧹 Web Search: {web_search.get('collections_cleaned', 0)} collections")
