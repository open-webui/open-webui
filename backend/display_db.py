...
def display_table(
    ...
    write_output(f"Table: {table_name}")
    write_output(f"{'='*80}")
    write_output(f"Total rows: {total_rows}" + (f" (showing {len(rows)})" if limit and len(rows) < total_rows else ""))
    write_output("")
    ...
    # Print header
    header = " | ".join([col_name.ljust(col_widths[col_name]) for col_name in column_names])
    write_output(header)
...
def display_database(
    ...
    try:
        ...
        if table_filter:
            matching_tables = [t for t in all_tables if table_filter.lower() in t.lower()]
        ...
        for table in tables_to_display:
            has_data = display_table(conn, table, limit=limit, show_empty=show_empty, output_file=output_file)
            if has_data:
                displayed_count += 1
...
def main():
    ...
    parser.add_argument(
        "--limit", "-l",
        type=int,
        help="Maximum number of rows to display per table"
    )
    parser.add_argument(
        "--no-empty",
        action="store_true",
