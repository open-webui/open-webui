...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "Cerebrum LLM UI")
if WEBUI_NAME != "Cerebrum LLM UI":
    WEBUI_NAME += " (Cerebrum LLM UI)"
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Cerebrum LLM UI + Ollama)
