...
def remove_migratehistory(db_path: str, skip_confirmation: bool = False) -> bool:
    ...
    try:
        ...
        if not skip_confirmation:
            ...
            print(
                "This table is only used by Peewee migrations, which are skipped when Alembic is detected."
            )
            response = (
...
def main():
    ...
    if db_abs:
        ...
        if "sqlite" not in DATABASE_URL.lower():
            ...
            print(
                "Set DB_ABS environment variable to specify the database path directly."
            )
            sys.exit(1)
