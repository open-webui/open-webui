...
IF "%WEBUI_DEBUG%"=="1" (
    ...
    python -m debugpy --listen 0.0.0.0:5678 --wait-for-client --log-to-stderr -m uvicorn open_webui.main:app --host "%HOST%" --port "%PORT%" --forwarded-allow-ips '*' --workers %UVICORN_WORKERS% --ws auto
) ELSE (
