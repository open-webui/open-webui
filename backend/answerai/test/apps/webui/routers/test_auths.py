...
class TestAuths(AbstractPostgresTest):
    ...
    def test_get_session_user(self):
        ...
        assert response.json() == {
            "id": "1",
            "name": "John Doe",
            "email": "john.doe@answerai.in",
            "role": "user",
            "profile_image_url": "/user.png",
        }

    def test_update_profile(self):
        from answerai.utils.auth import get_password_hash
    ...
    def test_signin(self):
        ...
        response = self.fast_api_client.post(
            ...
            json={"email": "john.doe@answerai.in", "password": "password"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == user.id
        assert data["name"] == "John Doe"
        assert data["email"] == "john.doe@answerai.in"
    ...
    def test_add_user(self):
        ...
        assert data["email"] == "john.doe2@answerai.in"
