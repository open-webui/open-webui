...
ENABLE_DIFY_API = os.environ.get("ENABLE_DIFY_API", "True").lower() == "true"

DIFY_API_BASE_URL = os.environ.get("DIFY_API_BASE_URL", "")

DIFY_API_KEYS = os.environ.get("DIFY_API_KEYS", "")
if DIFY_API_KEYS:
    DIFY_API_KEYS = [key.strip() for key in DIFY_API_KEYS.split(",") if key.strip()]
else:
    DIFY_API_KEYS = []

# Dify conversation memory mode
# 1: construct history_message attached to the message (default)
# 2: zero-width character mode
DIFY_CONVERSATION_MEMORY_MODE = int(os.environ.get("DIFY_CONVERSATION_MEMORY_MODE", "1"))

# Cache TTL for Dify model information (in seconds)
DIFY_MODELS_CACHE_TTL = int(os.environ.get("DIFY_MODELS_CACHE_TTL", "300"))
