...
| staging    | `staging-swept-chat` | https://staging.chat.swept.ai | https://workbench.swept.ai _(TODO)_ |
| demo       | `demo-swept-chat`    | https://demo.chat.swept.ai    | https://workbench.swept.ai _(TODO)_ |
...
**This environment was provisioned manually with `gcloud`, not Terraform.** Pioneer (`pioneer-insurance` project) is Terraform-managed and is a separate story. Do not introduce `.tf` files here — they would drift from the manual source of truth. When changing infra, update this runbook _and_ re-run the affected commands.
...
| Secret prefix      | _(none)_                  | `STAGING_`                   | `DEMO_`                   |
...
> Strongly prefer `scripts/bootstrap-gcp.sh` for this step — it generates each Postgres password locally, assembles the `*_OPENWEBUI_DATABASE_URL` secret, and writes it straight to Secret Manager in one pass. The password never lands on stdout, in shell history, or in scrollback.

If you must run it manually, do it as a single block so the password lives in a shell variable just long enough to flow into Secret Manager (and is gone when the subshell exits). The block intentionally does not `echo` the password:
...
for env in prod staging demo; do
  ...
  (
    ...
    gcloud sql users create "openwebui_${env}" --instance=swept-chat-db --password="${pw}" >/dev/null
    url="postgresql://openwebui_${env}:${pw}@/openwebui_${env}?host=/cloudsql/production-472518:us-central1:swept-chat-db"
    printf '%s' "${url}" | gcloud secrets create "${prefix}OPENWEBUI_DATABASE_URL" \
      --data-file=- --replication-policy=automatic >/dev/null
  )
done
```

`--edition=ENTERPRISE` is required: the project defaults new instances to Enterprise Plus, which rejects `db-custom-*` tiers and would force the much pricier `db-perf-optimized-N-*` family.

If a password is ever lost, it is unrecoverable — reset the SQL user's password via `gcloud sql users set-password ...`, regenerate the URL, and add a new version to the `*_OPENWEBUI_DATABASE_URL` secret.
...
### 5. Secret Manager — `WEBUI_SECRET_KEY` per env

Mirror workbench's prefix convention (`STAGING_` / `DEMO_`; prod has no prefix). Phase 3 already created the three `*_OPENWEBUI_DATABASE_URL` secrets; this phase covers the remaining `WEBUI_SECRET_KEY` (per env).
...
| `DATABASE_URL` (Phase 3) | `OPENWEBUI_DATABASE_URL` | `STAGING_OPENWEBUI_DATABASE_URL` | `DEMO_OPENWEBUI_DATABASE_URL` |

`WEBUI_SECRET_KEY` is a 32-byte JWT signing key. Pin one per env — rotating it invalidates all sessions in that env. Generate and store directly (no echoing):
...
for env in prod staging demo; do
  ...
  openssl rand -hex 32 | gcloud secrets create "${prefix}WEBUI_SECRET_KEY" \
    --data-file=- --replication-policy=automatic
done
```

Then grant the runtime SA `secretAccessor` on each of the six secrets created across Phases 3 and 5:

```sh
for s in WEBUI_SECRET_KEY STAGING_WEBUI_SECRET_KEY DEMO_WEBUI_SECRET_KEY \
         OPENWEBUI_DATABASE_URL STAGING_OPENWEBUI_DATABASE_URL DEMO_OPENWEBUI_DATABASE_URL; do
