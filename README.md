...
> [!WARNING]
> When using Docker or Podman to install Open WebUI, make sure to include the `-v open-webui:/app/backend/data` in your command. This step is crucial as it ensures your database is properly mounted and prevents any loss of data.
...
- **If Ollama is on your computer**, use this command (Docker or Podman):
...
- **If Ollama is on a Different Server**, use this command (Docker or Podman):
  ...
  - **To run Open WebUI with Nvidia GPU support**, use this command (Docker or Podman):
  ...
  ```bash
  podman run -d -p 3000:8080 --hooks prestart --hooks dir=/usr/local/nvidia --gpus all --add-host=host.containers.internal:host-gateway -v open-webui:/app/backend/data --name open-webui --restart always ghcr.io/open-webui/open-webui:cuda
  ```

### Installation for OpenAI API Usage Only

- **If you're only using OpenAI API**, use this command (Docker or Podman):
...
- **With GPU Support**:
  Utilize GPU resources by running the following command (Docker or Podman):
...
- **For CPU Only**:
  If you're not using a GPU, use this command (Docker or Podman):
...
We offer various installation alternatives, including non-Docker/Podman native installation methods, Docker Compose, Kustomize, and Helm. Visit our [Open WebUI Documentation](https://docs.openwebui.com/getting-started/) or join our [Discord community](https://discord.gg/5rJgQTnV4s) for comprehensive guidance.
...
If you're experiencing connection issues, it’s often due to the WebUI container not being able to reach the Ollama server at 127.0.0.1:11434 (host.docker.internal or host.containers.internal:11434) inside the container. Use the `--network=host` flag in your command to resolve this. Note that the port changes from 3000 to 8080, resulting in the link: `http://localhost:8080`.
...
### Keeping Your Docker/Podman Installation Up-to-Date

In case you want to update your local installation to the latest version, you can do it with [Watchtower](https://containrrr.dev/watchtower/):
...
```bash
podman run --rm --volume /run/podman/podman.sock:/run/podman/podman.sock containrrr/watchtower --run-once open-webui
```
...
```bash
podman run -d -p 3000:8080 -v open-webui:/app/backend/data --name open-webui --add-host=host.containers.internal:host-gateway --restart always ghcr.io/open-webui/open-webui:dev
```
