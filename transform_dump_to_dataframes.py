...
class PostgresDumpParser:
    ...
    def _convert_custom_to_sql(self, dump_file: str) -> str:
        ...
        try:
            subprocess.run(['pg_restore', '--version'], 
                         capture_output=True, check=True)
        except (subprocess.CalledProcessError, FileNotFoundError):
            print("\nError: pg_restore is not available.")
            print("Please install PostgreSQL client tools, or convert the dump manually:")
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                result = subprocess.run(
                    ['pg_restore', '--no-owner', '--no-privileges', '-f', sql_file, dump_file],
                    capture_output=True,
                    text=True
                )
                if result.returncode != 0:
...
class DataTransformer:
    ...
    def _convert_timestamps(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        timestamp_cols = [col for col in df.columns if 'at' in col.lower() or 'time' in col.lower()]
    ...
    def _transform_user(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'info' in df.columns:
            df['info_parsed'] = df['info'].apply(lambda x: x if isinstance(x, dict) else (json.loads(x) if isinstance(x, str) and x.startswith('{') else None))
        
        if 'settings' in df.columns:
            df['settings_parsed'] = df['settings'].apply(lambda x: x if isinstance(x, dict) else (json.loads(x) if isinstance(x, str) and x.startswith('{') else None))
    ...
    def _transform_chat(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'chat' in df.columns:
            df['chat_parsed'] = df['chat'].apply(lambda x: x if isinstance(x, dict) else (json.loads(x) if isinstance(x, str) and x.startswith('{') else None))
            
            # Extract message count if available
            if df['chat_parsed'].notna().any():
                df['message_count'] = df['chat_parsed'].apply(
                    lambda x: len(x.get('history', {}).get('messages', {})) if isinstance(x, dict) else 0
                )
        
        # Parse meta field
        if 'meta' in df.columns:
            df['meta_parsed'] = df['meta'].apply(lambda x: x if isinstance(x, dict) else (json.loads(x) if isinstance(x, str) and x.startswith('{') else None))
        ...
        for col in ['data', 'meta']:
            if col in df.columns:
                df[f'{col}_parsed'] = df[col].apply(
                    lambda x: x if isinstance(x, dict) else (json.loads(x) if isinstance(x, str) and x.startswith('{') else None)
                )
    ...
    def _transform_moderation_session(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Parse JSON fields
        json_cols = ['strategies', 'custom_instructions', 'highlighted_texts', 'refactored_response', 'session_metadata']
        for col in json_cols:
            if col in df.columns:
                df[f'{col}_parsed'] = df[col].apply(
                    lambda x: x if isinstance(x, dict) or isinstance(x, list) else (json.loads(x) if isinstance(x, str) and (x.startswith('{') or x.startswith('[')) else None)
                )
